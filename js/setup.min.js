...
content:'<table class="setup-tab"><tr><td class="label">Наименование:<td><input id="name" type="text" maxlength="50" /><tr><td class="label topi">Описание:<td><textarea id="about"></textarea><tr><td class="label topi">Виды платежей:<td><input type="hidden" id="types" /></table>',submit:a});$("#name").focus().keyEnter(a);$("#types")._select({width:218,multiselect:1,spisok:INCOME_SPISOK})}).on("click","#setup_invoice .img_edit",function(){function a(){var a={op:"setup_invoice_edit",id:e,name:$("#name").val(),
about:$("#about").val(),types:$("#types").val()};a.name?(f.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),f.close(),_msg("Сохранено!")):(f.abort(),b(a.text))},"json")):(b("Не указано наименование"),$("#name").focus())}function b(a){f.bottom.vkHint({msg:"<SPAN class=red>"+a+"</SPAN>",top:-47,left:100,indent:50,show:1,remove:1})}for(var c=$(this);"TR"!=c[0].tagName;)c=c.parent();var e=c.attr("val"),d=c.find(".name div").html(),g=c.find(".name pre").html(),c=c.find(".type_id").val(),
...
submit:a});$("#name").focus().keyEnter(a)}).on("click","#setup_income .img_edit",function(){function a(){var a={op:"setup_income_edit",id:c,name:$("#name").val()};a.name?(e.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),e.close(),_msg("Сохранено!"),sortable()):e.abort()},"json")):(e.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}for(var b=$(this);"DD"!=b[0].tagName;)b=b.parent();var c=
b.attr("val"),b='<table class="setup-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="100" value="'+b.find(".name").html()+'" /></table>',e=_dialog({top:60,width:440,head:"Редактирование вида платежа",content:b,butSubmit:"Сохранить",submit:a});$("#name").focus().keyEnter(a)}).on("click","#setup_income .img_del",function(){var a=$(this),b=_dialog({top:90,width:300,head:"Удаление вида платежа",content:"<center><b>Подтвердите удаление вида платежа.</b></center>",
...
$("#name").focus().keyEnter(a);$("#show_worker")._check()}).on("click","#setup_expense .img_edit",function(){function a(){var a={op:"setup_expense_edit",id:c,name:$("#name").val(),show_worker:$("#show_worker").val()};a.name?(d.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),d.close(),_msg("Сохранено!"),sortable()):d.abort()},"json")):(d.bottom.vkHint({msg:'<span class="red">Не указано наименование</span>',top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}
...
"#setup_zayavrashod .img_edit",function(){function a(){var a={op:"setup_zayavrashod_edit",id:e,name:$("#name").val(),show_txt:$("#show_txt").val(),show_worker:$("#show_worker").val()};a.name?(f.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),f.close(),_msg("Сохранено!"),sortable()):f.abort()},"json")):(f.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}function b(a,b){1==a&&("show_txt"==
b?$("#show_worker")._check(0):$("#show_txt")._check(0))}for(var c=$(this);"DD"!=c[0].tagName;)c=c.parent();var e=c.attr("val"),d=c.find(".name").html(),g=c.find(".txt").html()?1:0,c=c.find(".worker").html()?1:0,f=_dialog({top:60,width:440,head:"Редактирование категории расхода заявки",content:'<table style="border-spacing:10px"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="50" style="width:210px" value="'+d+'" /><tr><td class="label r">Текстовое поле:<td><input id="show_txt" type="hidden" value="'+
...
post:$("#post").val()},c=$(this);b.first_name?b.last_name?(c.addClass("busy"),$.post(AJAX_MAIN,b,function(a){c.removeClass("busy");a.success&&_msg("Сохранено.")},"json")):(a("Не указана фамилия"),$("#last_name").focus()):(a("Не указано имя"),$("#first_name").focus())}),$(".pin-clear").click(function(){var a={op:"setup_worker_pinclear",viewer_id:RULES_VIEWER_ID},b=$(this);b.hasClass("busy")||(b.addClass("busy"),$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");a.success&&_msg("Пин-код сброшен.")},
"json"))}),$("#rules_bonus")._dropdown({spisok:[{uid:0,title:"по дате выполнения заявок"},{uid:1,title:"по дате внесения заявок"}]}),$("#rules_appenter")._check(function(a){$(".app-div")[(0==a?"add":"remove")+"Class"]("dn");$("#rules_worker")._check(0);$("#rules_rules")._check(0);$("#rules_rekvisit")._check(0);$("#rules_product")._check(0);$("#rules_income")._check(0);$("#rules_zayavrashod")._check(0);$("#rules_historyshow")._check(0);$("#rules_money")._dropdown(0)}),$("#rules_money")._dropdown({spisok:[{uid:0,
title:"только свои"},{uid:1,title:"все платежи"}]}),$(".rules-save").click(function(){var a={op:"setup_worker_rules_save",viewer_id:RULES_VIEWER_ID,rules_appenter:$("#rules_appenter").val(),rules_worker:$("#rules_worker").val(),rules_rules:$("#rules_rules").val(),rules_rekvisit:$("#rules_rekvisit").val(),rules_product:$("#rules_product").val(),rules_income:$("#rules_income").val(),rules_zayavrashod:$("#rules_zayavrashod").val(),rules_historyshow:$("#rules_historyshow").val(),rules_money:$("#rules_money").val()},
b=$(this);b.hasClass("busy")||(b.addClass("busy"),$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");a.success&&_msg("Права сохранены.")},"json"))}),$(".dop-save").click(function(){var a={op:"setup_worker_dop_save",viewer_id:RULES_VIEWER_ID,rules_bonus:$("#rules_bonus").val(),rules_cash:$("#rules_cash").val(),rules_getmoney:$("#rules_getmoney").val(),rules_nosalary:$("#rules_nosalary").val()},b=$(this);b.hasClass("busy")||(b.addClass("busy"),$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");
a.success&&_msg("Дополнительные настройки сохранены.")},"json"))}));$("#setup_rekvisit").length&&$(".vkButton").click(function(){var a=$(this),b={op:"setup_rekvisit",org_name:$("#org_name").val(),ogrn:$("#ogrn").val(),inn:$("#inn").val(),kpp:$("#kpp").val(),yur_adres:$("#yur_adres").val(),telefon:$("#telefon").val(),ofice_adres:$("#ofice_adres").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){a.removeClass("busy");b.success&&_msg("Информация сохранена.")},"json")})});
