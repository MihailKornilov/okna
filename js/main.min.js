...
0!=c.sum)if(c.avans&&!REGEXP_CENA.test(c.avans))h("Некорректно указан авансовый платёж","avans",b);else{if("reneg"!=a||c.reason)return c;h("Не указана причина перезаключения договора","reason",b)}else h("Некорректно указана сумма по договору","sum",b);else h("Некорректно указан номер договора","nomer",b);else h("Не указано Фио клиента","fio",b);return!1}function c(){var a=p[l-1],a='<tr><td class="label">'+l+'-й платёж:<td><input type="text" class="cutsum" id="i'+l+'" maxlength="7" value="'+(a?a[0]:
"")+'"> руб. <input type="hidden" id="d'+l+'" value="'+(a?a[1]:"")+'">';$("#cut-add").before(a);1!=l||p[0]||$("#i1").val($("#cut-itog").html());$("#i"+l).focus().keyup(d);$("#d"+l)._calendar();l++}function d(){var a=$(".cutsum"),b=0,c,e=[];for(g=0;g<a.length;g++)if((c=$.trim(a.eq(g).val()))&&0!=c){if(!REGEXP_CENA.test(c)){b=!1;break}b+=1*c;e.push(1*c+":"+a.eq(g).next().find("input").val())}!1===b&&(b=Math.round(100*b)/100);$("#cut-itog").html(b);return!1===b?"error":e}function e(){var a=d();"error"==
a?r.bottom.vkHint({msg:'<span class="red">Некорректно заполнено поле платежа</span>',top:-47,left:84,indent:50,show:1,remove:1}):(DOG.cut=a.join(),f(),r.close())}function f(){var a=DOG.cut?DOG.cut.split(",").length:0;$("#cut").html(a?"Указан"+_end(a,["а","ы"])+" дат"+_end(a,["а","ы"])+" для "+a+_end(a,["-го","-х","-и"])+" платеж"+_end(a,["а","ей"])+'. <u>Изменить</u><div class="img_del'+_tooltip("Отменить разбивку",-61)+"</div>":m);a&&$("#cut .img_del").click(function(a){a.stopPropagation();DOG.cut=
...
(c++,d+=1*h.parent().find(".sum").html().replace(" ",""),f.push(h.parent().attr("val"))):e=!1}$(".income_choice_sum").html(c?"Выбран"+_end(c,["","о"])+" <b>"+c+"</b> платеж"+_end(c,["","а","ей"])+" на сумму <b>"+d+"</b> руб.":"");$("#money_all")._check(e);return{count:c,ids:f.join(),sum:d}},expenseFilter=function(){for(var a=[],b=$("#monthList input"),c=1;12>=c;c++)1==b.eq(c-1).val()&&a.push(c);return{op:"expense_spisok",category:$("#category").val(),worker:$("#worker").val(),invoice_id:$("#invoice_id").val(),
...
var b=_dialog({width:400,head:"Перенос заказа в установку",content:'<table class="_dialog-tab"><tr><td class="label">Клиент:<td>'+ZAYAV.client_fio+'<tr><td class="label">Адрес установки:<td><INPUT type="text" id="adres" maxlength="100" value="'+ZAYAV.adres+'" /><INPUT type="hidden" id="homeadres" /></table>',butSubmit:"Перенести",submit:a});$("#adres").focus().keyEnter(a);$("#homeadres")._check({func:function(){$("#adres").val(ZAYAV.client_adres)}});$("#homeadres_check").vkHint({msg:"Совпадает с адресом проживания клиента",
