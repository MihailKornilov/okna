...
$.fn.productList=function(a){function b(a){var d=e+f,b=d+"id",g=d+"subid",h=d+"count";m.before('<table id="ptab'+f+'" class="ptab" val="'+f+'"><tr><td class="td"><input type="hidden" id="'+b+'" value="'+(a[0]||0)+'" /><input type="hidden" id="'+g+'" value="'+(a[1]||0)+'" /><td class="td"><input type="text" id="'+h+'" value="'+(a[2]||"")+'" class="count" maxlength="3" /> шт.'+(1<f?'<div class="img_del"></div>':"")+"</table>");var k=$("#ptab"+f);k.find(".img_del").click(function(){k.remove()});$("#"+
b)._select({width:119,title0:"Не указано",spisok:PRODUCT_SPISOK,func:function(a){$("#"+g)._select("remove").val(0);0<a&&PRODUCT_SUB_SPISOK[a]&&c(a,g,h);$("#"+h).val(0<a?1:"").focus()}});c(a[0]||0,g,h);f++}function c(a,d,b){0!=a&&PRODUCT_SUB_SPISOK[a]&&$("#"+d)._select({width:120,title0:"Подвид не указан",spisok:PRODUCT_SUB_SPISOK[a],func:function(){$("#"+b).focus()}})}var d=$(this),e=d.attr("id"),f=1,g;if("string"==typeof a&&"get"==a){a=d.find(".ptab");d=[];for(g=0;g<a.length;g++){var h=a.eq(g),k=
e+h.attr("val"),h=$("#"+k+"id").val(),l=$("#"+k+"subid").val(),k=$("#"+k+"count").val();if(0!=h){if(!REGEXP_NUMERIC.test(k)||0==k)return"count_error";d.push(h+":"+l+":"+k)}}return 0==d.length?!1:d.join()}d.html('<div class="_product-list"><a class="add">Добавить поле</a></div>');var m=d.find(".add");m.click(b);if("object"==typeof a)for(g=0;g<a.length;g++)b(a[g]);else b([]);return d};
$.fn.zayavRashod=function(a){function b(a){var c=d+e,f=c+"cat",g=c+"worker";p.append('<table id="ptab'+e+'" class="ptab" val="'+e+'"><tr><td><input type="hidden" id="'+f+'" value="'+(a[0]||0)+'" /><td class="tddop">'+(a[0]&&ZAYAVRASHOD_TXT_ASS[a[0]]?'<input type="text" class="zrtxt" placeholder="описание не указано" tabindex="'+(10*e-1)+'" value="'+a[1]+'" />':"")+(a[0]&&ZAYAVRASHOD_WORKER_ASS[a[0]]?'<input type="hidden" id="'+g+'" value="'+a[1]+'" />':"")+'<td class="tdsum'+(a[0]?"":" dn")+'"><input type="text" class="zrsum" maxlength="6" tabindex="'+
10*e+'" value="'+(a[2]||"")+'" />руб.</table>');var h=$("#ptab"+e),k=h.find(".tddop"),l=h.find(".zrsum");a[0]&&ZAYAVRASHOD_WORKER_ASS[a[0]]&&$("#"+g)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}});$("#"+f)._select({width:120,title0:"Категория",spisok:ZAYAVRASHOD_SPISOK,func:function(a){h.find(".tdsum")[(0<a?"remove":"add")+"Class"]("dn");ZAYAVRASHOD_TXT_ASS[a]?(k.html('<input type="text" class="zrtxt" placeholder="описание не указано" tabindex="'+(10*e-11)+
'" />'),k.find(".zrtxt").focus()):(ZAYAVRASHOD_WORKER_ASS[a]?(k.html('<input type="hidden" id="'+g+'" />'),$("#"+g)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}})):k.html(""),l.focus());l.val("");0<a&&!h.next().hasClass("ptab")&&b([])}});e++}var c=$(this),d=c.attr("id"),e=1,f;if("string"==typeof a&&"get"==a){a=c.find(".ptab");c=[];for(f=0;f<a.length;f++){var g=a.eq(f),h=d+g.attr("val"),k=$("#"+h+"cat").val();$("#"+h+"worker").val();var l=g.find(".zrsum").val(),
m="";if(0!=k){if(!REGEXP_NUMERIC.test(l)||0==l)return"sum_error";ZAYAVRASHOD_TXT_ASS[k]?m=g.find(".zrtxt").val():ZAYAVRASHOD_WORKER_ASS[k]&&(m=$("#"+h+"worker").val());c.push(k+":"+m+":"+l)}}return c.join()}c.html('<div class="_zayav-rashod"></div>');var p=c.find("._zayav-rashod");if("object"==typeof a)for(f=0;f<a.length;f++)b(a[f]);b([]);return c};
...
a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".invoice_set",function(){function a(){var a=[];if(f)for(n=0;n<e;n++){var d=h.eq(n).val();if(REGEXP_CENA.test(d))a.push(CASH[n].id+"="+1*d.replace(",","."));else{b("Некорректно указана сумма");h.eq(n).focus();return}}a={op:"invoice_set",invoice_id:c,sum:f?0:$("#sum").val(),cash:a.join(":")};f||REGEXP_CENA.test(a.sum)?(g.process(),$.post(AJAX_MAIN,a,function(a){a.success?
($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),g.close(),_msg("Баланс установлен.")):g.abort()},"json")):(b("Некорректно указана сумма"),$("#sum").focus())}function b(a){g.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:72})}window.CASH||(window.CASH=[]);var c=$(this).attr("val"),d='<table class="_dialog-tab">',e=CASH.length,f="1"==c&&!!e;if(f){for(n=0;n<e;n++)d+='<tr><td class="label">'+CASH[n].name+':<td><INPUT type="text" class="money" maxlength="11" value="'+
CASH[n].sum+'"> руб.';d+='<tr><td class="label">Счёт <b>'+INVOICE_NAME+'</b>:<td><b id="summa">0</b> руб.'}else d+='<tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="11"> руб.';var g=_dialog({width:320,head:"Установка начального баланса счёта",content:d+"</table>",butSubmit:"Установить",submit:a}),h=g.content.find("input");h.eq(0).focus();h.keyEnter(a);f&&h.keyup(function(){var a=0;for(n=0;n<e;n++){var b=h.eq(n).val();if(REGEXP_CENA.test(b))a+=1*b.replace(",",".");
else{a=0;break}}$("#summa").html(a)})}).on("click","#report.invoice .img_note",function(){var a=_dialog({top:20,width:570,head:"История операций со счётом",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"invoice_history",invoice_id:$(this).attr("val")};$.post(AJAX_MAIN,b,function(b){b.success?a.content.html(b.html):a.loadError()},"json")}).on("click",".income-add",function(){function a(){var a={op:"income_add",from:OPL.from,type:$("#income_opl").val(),sum:$("#sum").val(),zayav_id:$("#zayav_id").val()||
...
break;case "income":incomeSpisok()}else c.abort()},"json")):(b("Необходимо указать комментарий"),$("#prim").focus()):(b("Некорректно указана сумма."),$("#sum").focus())}function b(a){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:135})}var c=_dialog({top:60,width:440,head:"Внесение платежа",content:'<table class="income-add-tab">'+("income"!=OPL.from?'<tr><td class="label">Клиент:<td>'+OPL.client_fio+'<tr><td class="label">Заявка:<td><input type="hidden" id="zayav_id" value="'+
(OPL.zayav_id?OPL.zayav_id:0)+'">'+(OPL.zayav_id?"<b>№"+OPL.zayav_id+"</b>":""):"")+'<tr><td class="label">Вид платежа:<td><input type="hidden" id="income_opl"><a href="'+URL+'&p=setup&d=income" class="img_edit" title="Перейти к настройке видов платежей"></a><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" maxlength="11"> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="100"></table>',submit:a});$("#sum").focus();$("#sum,#prim").keyEnter(a);
OPL.zayav_spisok&&$("#zayav_id")._select({width:210,title0:"Не указана",spisok:OPL.zayav_spisok});$("#income_opl")._select({width:180,title0:"Не указан",spisok:INCOME_SPISOK,func:function(a){$("#sum").focus()}})}).on("click",".income-del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"income_del",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){a.removeClass("deleting");b.success&&a.addClass("deleted")},"json")}}).on("click",
...
a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".expense #monthList div",expenseSpisok).on("click",".expense ._next",function(){var a=$(this),b=expenseFilter();b.page=a.attr("val");a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".expense .img_del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"expense_del",
id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){a.removeClass("deleting");b.success&&a.addClass("deleted")},"json")}}).on("click",".expense .img_rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"expense_rest",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){b.success&&a.removeClass("deleted")},"json")}).on("click",".expense .img_edit",function(){function a(a){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-47,left:101})}for(var b=
$(this);"TR"!=b[0].tagName;)b=b.parent();var c=_dialog({width:380,head:"Редактирование расхода",load:1,butSubmit:"Сохранить",submit:function(){var b={id:d,op:"expense_edit",category:1*$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};b.category||b.about?b.invoice?REGEXP_NUMERIC.test(b.sum)?(c.process(),$.post(AJAX_MAIN,b,function(a){a.success?(c.close(),_msg("Расход изменён."),expenseSpisok()):c.abort()},"json")):(a("Некорректно указана сумма."),
$("#sum").focus()):a("Укажите с какого счёта производится оплата."):(a("Выберите категорию или укажите описание."),$("#about").focus())}}),d=b.attr("val");$.post(AJAX_MAIN,{op:"expense_get",id:d},function(a){c.content.html('<table id="expense-add-tab"><tr><td class="label">Категория:<td><input type="hidden" id="cat" value="'+a.category+'"><tr class="tr-work '+(EXPENSE_WORKER_ASS[a.category]?"":"dn")+'"><td class="label">Сотрудник:<td><input type="hidden" id="work" value="'+a.worker+'"><tr><td class="label">Описание:<td><input type="text" id="about" maxlength="150" value="'+
...
about:$("#about").val(),types:$("#types").val()};a.name?(g.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),g.close(),_msg("Сохранено!")):(g.abort(),b(a.text))},"json")):(b("Не указано наименование"),$("#name").focus())}function b(a){g.bottom.vkHint({msg:"<SPAN class=red>"+a+"</SPAN>",top:-47,left:100,indent:50,show:1,remove:1})}for(var c=$(this);"TR"!=c[0].tagName;)c=c.parent();var d=c.attr("val"),e=c.find(".name div").html(),f=c.find(".name pre").html(),c=c.find(".type_id").val(),
g=_dialog({top:60,width:400,head:"Редактирование данных счёта",content:'<table class="setup-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="100" value="'+e+'" /><tr><td class="label r top">Описание:<td><textarea id="about">'+f+'</textarea><tr><td class="label topi">Виды платежей:<td><input type="hidden" id="types" value="'+c+'" /></table>',butSubmit:"Сохранить",submit:a});$("#name").focus().keyEnter(a);$("#types")._select({width:218,multiselect:1,spisok:INCOME_SPISOK})}).on("click",
...
"#setup_zayavrashod .img_edit",function(){function a(){var a={op:"setup_zayavrashod_edit",id:d,name:$("#name").val(),show_txt:$("#show_txt").val(),show_worker:$("#show_worker").val()};a.name?(g.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),g.close(),_msg("Сохранено!"),sortable()):g.abort()},"json")):(g.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}function b(a,b){1==a&&("show_txt"==
b?$("#show_worker")._check(0):$("#show_txt")._check(0))}for(var c=$(this);"DD"!=c[0].tagName;)c=c.parent();var d=c.attr("val"),e=c.find(".name").html(),f=c.find(".txt").html()?1:0,c=c.find(".worker").html()?1:0,g=_dialog({top:60,width:440,head:"Редактирование категории расхода заявки",content:'<table style="border-spacing:10px"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="50" style="width:210px" value="'+e+'" /><tr><td class="label r">Текстовое поле:<td><input id="show_txt" type="hidden" value="'+
f+'" /><tr><td class="label r">Список сотрудников:<td><input id="show_worker" type="hidden" value="'+c+'" /></table>',butSubmit:"Сохранить",submit:a});$("#name").focus().keyEnter(a);$("#show_txt")._check({func:b});$("#show_worker")._check({func:b})}).on("click","#setup_zayavrashod .img_del",function(){var a=$(this),b=_dialog({top:90,width:300,head:"Удаление категории расхода заявки",content:"<center><b>Подтвердите удаление<br />категории расхода заявки.</b></center>",butSubmit:"Удалить",submit:function(){for(;"DD"!=
...
a?"block":"none");$("#comments").css("display","comm"==a?"block":"none");$("#histories").css("display","hist"==a?"block":"none")}),$(".cedit").click(function(){function a(){var d={op:"client_edit",client_id:CLIENT.id,fio:$("#fio").val(),telefon:$("#telefon").val(),adres:$("#adres").val(),pasp_seria:$("#pasp_seria").val(),pasp_nomer:$("#pasp_nomer").val(),pasp_adres:$("#pasp_adres").val(),pasp_ovd:$("#pasp_ovd").val(),pasp_data:$("#pasp_data").val()};d.fio?(b.process(),$.post(AJAX_MAIN,d,function(a){a.success?
...
1).val()){c=1;break}for(b=1;12>=b;b++)$("#c"+b)._check(c);expenseSpisok()}}));$("#report.invoice").length&&$(".transfer").click(function(){function a(){var b={op:"invoice_transfer",from:1*$("#from").val(),to:1*$("#to").val(),sum:$("#sum").val(),cash:window.CASH_SPISOK?1:0,prim:$("#prim").val()};b.from?b.to?b.from==b.to?c("Выберите другой счёт"):REGEXP_CENA.test(b.sum)&&0!=b.sum?(f.process(),$.post(AJAX_MAIN,b,function(a){a.success?($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),$("#transfer-spisok").html(a.t),
f.close(),_msg("Перевод произведён.")):f.abort()},"json")):(c("Некорректно введена сумма"),$("#sum").focus()):c("Выберите счёт-получатель"):c("Выберите счёт-отправитель")}function c(a){f.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:-47,left:82,indent:50,show:1,remove:1})}$(this);var f=_dialog({head:"Перевод между счетами",content:'<table class="invoice-transfer"><tr><td class="label">Со счёта:<td><input type="hidden" id="from" /><tr><td class="label">На счёт:<td><input type="hidden" id="to" /><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" /> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="50" /></table>',
