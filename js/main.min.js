...
10*e+'" value="'+(a[2]||"")+'" />руб.</table>');var h=$("#ptab"+e),k=h.find(".tddop"),l=h.find(".zrsum");a[0]&&ZAYAVRASHOD_WORKER_ASS[a[0]]&&$("#"+f)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}});$("#"+g)._select({width:120,title0:"Категория",spisok:ZAYAVRASHOD_SPISOK,func:function(a){h.find(".tdsum")[(a?"remove":"add")+"Class"]("dn");ZAYAVRASHOD_TXT_ASS[a]?(k.html('<input type="text" class="zrtxt" tabindex="'+(10*e-11)+'" />'),k.find(".zrtxt").focus()):(ZAYAVRASHOD_WORKER_ASS[a]?
(k.html('<input type="hidden" id="'+f+'" />'),$("#"+f)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}})):k.html(""),l.focus());l.val("");a&&!h.next().hasClass("ptab")&&b([])}});e++}var c=$(this),d=c.attr("id"),e=1,g;if("string"==typeof a&&"get"==a){a=c.find(".ptab");c=[];for(g=0;g<a.length;g++){var f=a.eq(g),k=d+f.attr("val"),h=$("#"+k+"cat").val();$("#"+k+"worker").val();var m=f.find(".zrsum").val(),l="";if(0!=h){if(!REGEXP_NUMERIC.test(m)||0==m)return"sum_error";
ZAYAVRASHOD_TXT_ASS[h]?l=f.find(".zrtxt").val():ZAYAVRASHOD_WORKER_ASS[h]&&(l=$("#"+k+"worker").val());c.push(h+":"+l+":"+m)}}return c.join()}c.html('<div class="_zayav-rashod"></div>');var n=c.find("._zayav-rashod");if("object"==typeof a)for(g=0;g<a.length;g++)b(a[g]);b([]);return c};
...
submit:a});$("#name").focus().keyEnter(a)}).on("click","#setup_prihodtype .img_edit",function(){function a(){var a={op:"setup_prihodtype_edit",id:c,name:$("#name").val()};a.name?(d.process(),$.post(AJAX_MAIN,a,function(a){a.success?($(".spisok").html(a.html),d.close(),_msg("Сохранено!"),sortable()):d.abort()},"json")):(d.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:131,indent:50,show:1,remove:1}),$("#name").focus())}for(var b=$(this);"DD"!=b[0].tagName;)b=b.parent();
var c=b.attr("val"),b='<table class="setup-tab"><tr><td class="label r">Наименование:<td><input id="name" type="text" maxlength="100" value="'+b.find(".name").html()+'" /></table>',d=_dialog({top:60,width:440,head:"Редактирование вида платежа",content:b,butSubmit:"Сохранить",submit:a});$("#name").focus().keyEnter(a)}).on("click","#setup_prihodtype .img_del",function(){var a=$(this),b=_dialog({top:90,width:300,head:"Удаление вида платежа",content:"<center><b>Подтвердите удаление вида платежа.</b></center>",
butSubmit:"Удалить",submit:function(){for(;"DD"!=a[0].tagName;)a=a.parent();var c={op:"setup_prihodtype_del",id:a.attr("val")};b.process();$.post(AJAX_MAIN,c,function(a){a.success?($(".spisok").html(a.html),b.close(),_msg("Удалено!"),sortable()):b.abort()},"json")}})}).on("click","#setup_zayavrashod .add",function(){function a(){var a={op:"setup_zayavrashod_add",name:$("#name").val(),show_txt:$("#show_txt").val(),show_worker:$("#show_worker").val()};a.name?(c.process(),$.post(AJAX_MAIN,a,function(a){a.success?
