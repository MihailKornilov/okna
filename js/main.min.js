...
_dialog({width:250,head:"Подтверждение пин-кода",content:'<table class="setup-tab"><tr><td colspan="2"><div class="_info">Истекло время действия пин-кода. Требуется подтверждение.</div><tr><td class="label">Пин-код:<td><input id="tpin" type="password" maxlength="10" /></table>',butSubmit:"Подтвердить",butCancel:"",submit:d});$("#tpin").focus().keyEnter(d)}}).on("change","._attach input",function(){_\u0441ookie("_attached",0);for(var a=$(this);!a.hasClass("_attach");)a=a.parent();var b=a.find("form"),
c=a.find(".form"),d=setInterval(function(){var e=_\u0441ookie("_attached");0<e&&(clearInterval(d),1==e?(e={op:"attach_get",type:b.find(".type").val(),zayav_id:b.find(".zayav_id").val()},$.post(AJAX_MAIN,e,function(e){c.removeClass("_busy");e.success&&(a.find(".files").html(e.files),b.html(e.form))},"json")):(c.removeClass("_busy"),c.next(".red").remove(),c.after('<div class="red">Некорректный файл.</div>'),c.next(".red").fadeOut(4E3)))},500);c.addClass("_busy");b.submit()}).on("click","._attach .img_minidel",
