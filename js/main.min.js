...
c+'" value="'+d[c]+'">';$("#preview-form").html(b).submit()}});$("#reason").autosize()},setupRulesSet=function(a,b){$.post(AJAX_MAIN,{op:"setup_rules_set",viewer_id:RULES_VIEWER_ID,value:b,action:a},function(){},"json")};
...
$(document).on("click","._calendarFilter .week-num,._calendarFilter .on,._calendarFilter .data a",function(){for(var a=$(this),b=a.hasClass("week-num")?a.parent():a,c=b.hasClass("sel"),d=b;!d.hasClass("_calendarFilter");)d=d.parent();c||d.find(".sel").removeClass("sel");b[(c?"remove":"add")+"Class"]("sel");window._calendarFilter&&_calendarFilter(c?"":a.attr("val"))}).on("click","._calendarFilter .ch",function(){for(var a=$(this),b={op:"calendar_filter_ch",month:a.attr("val")};!a.hasClass("_calendarFilter");)a=
a.parent();a.hasClass("busy")||(a.addClass("busy"),b.func=a.find(".func").val(),$.post(AJAX_MAIN,b,function(b){a.removeClass("busy");b.success&&a.find(".content").html(b.html)},"json"))}).on("change","._attach input",function(){setCookie("_attached",0);for(var a=$(this);!a.hasClass("_attach");)a=a.parent();var b=a.find("form"),c=a.find(".form"),d=setInterval(function(){var e=getCookie("_attached");0<e&&(clearInterval(d),1==e?(e={op:"attach_get",type:b.find(".type").val(),zayav_id:b.find(".zayav_id").val()},
$.post(AJAX_MAIN,e,function(d){c.removeClass("_busy");d.success&&(a.find(".files").html(d.files),b.html(d.form))},"json")):(c.removeClass("_busy"),c.next(".red").remove(),c.after('<div class="red">Некорректный файл.</div>'),c.next(".red").fadeOut(4E3)))},500);c.addClass("_busy");b.submit()}).on("click","._attach .img_minidel",function(){var a=$(this),b={op:"attach_del",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){b.success&&(a.prev().remove(),a.remove())},"json")}).on("click",".oplata-add",function(){function a(){var a=
{op:"oplata_add",from:OPL.from,type:$("#income_id").val(),sum:$("#sum").val(),zayav_id:$("#zayav_id").val(),client_id:OPL.client_id,prim:$.trim($("#prim").val())};0==a.type?b("Не указан вид платежа"):REGEXP_NUMERIC.test(a.sum)?0!=a.zayav_id||a.prim?(c.process(),$.post(AJAX_MAIN,a,function(a){if(a.success)switch(c.close(),_msg("Платёж успешно внесён!"),OPL.from){case "client":$("#income_spisok").html(a.html);$(".left:first").html(a.balans);break;case "zayav":$("#income_spisok").html(a.html)}else c.abort()},
"json")):b("Если не выбрана заявка, необходимо указать примечание"):(b("Некорректно указана сумма."),$("#sum").focus())}function b(a){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:135})}var c=_dialog({top:60,width:440,head:"Внесение платежа",content:'<table class="oplata-add-tab"><tr><td class="label">Клиент:<td>'+OPL.client_fio+'<tr><td class="label">Заявка:<td><input type="hidden" id="zayav_id" value="'+(OPL.zayav_id?OPL.zayav_id:0)+'">'+(OPL.zayav_id?
"<b>№"+OPL.zayav_id+"</b>":"")+'<tr><td class="label">Вид платежа:<td><input type="hidden" id="income_id" value="0"><a href="'+URL+'&p=setup&d=income" class="img_edit" title="Перейти к настройке видов платежей"></a><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" maxlength="7"> руб.<tr><td class="label">Примечание:<em>(не обязательно)</em><td><input type="text" id="prim"></table>',submit:a});$("#sum").focus();$("#sum,#prim").keyEnter(a);OPL.zayav_spisok&&$("#zayav_id")._select({width:210,
title0:"Не указана",spisok:OPL.zayav_spisok});$("#income_id")._select({width:180,title0:"Не указан",spisok:PRIHOD_SPISOK,func:function(a){$("#sum").focus()}})}).on("click",".oplata-del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"oplata_del",id:a.attr("val")};$.post(AJAX_MAIN,b,function(c){a.removeClass("deleting");c.success&&(a.after('<tr class="deleted" val="'+b.id+'"><td colspan="4"><div>Платёж удалён. <a class="oplata-rest">Восстановить</a></div>'),
a.addClass("dn"))},"json")}}).on("click",".oplata-rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"oplata_rest",id:a.attr("val")},c=a.find("div");c.hasClass("_busy")||(c.addClass("_busy"),$.post(AJAX_MAIN,b,function(b){c.removeClass("busy");b.success&&(a.prev().removeClass("dn"),a.remove())},"json"))}).on("click",".accrual-del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"accrual_del",id:a.attr("val")};
$.post(AJAX_MAIN,b,function(c){a.removeClass("deleting");c.success&&(a.after('<tr class="deleted" val="'+b.id+'"><td colspan="4"><div>Начисление удалено. <a class="accrual-rest">Восстановить</a></div>'),a.addClass("dn"))},"json")}}).on("click",".accrual-rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"accrual_rest",id:a.attr("val")},c=a.find("div");c.hasClass("_busy")||(c.addClass("_busy"),$.post(AJAX_MAIN,b,function(b){c.removeClass("busy");b.success&&(a.prev().removeClass("dn"),
a.remove())},"json"))}).on("click","#client .ajaxNext",function(){if(!$(this).hasClass("busy")){var a=$(this),b=clientFilter();b.op="client_next";b.page=a.attr("val");a.addClass("busy");$.post(AJAX_MAIN,b,function(b){b.success?(a.remove(),$("#client .left").append(b.spisok)):a.removeClass("busy")},"json")}}).on("click",".zayav_add",function(){_dialog({width:370,top:30,head:"Выберите категорию заявки",content:'<div class="zayav-add"><div class="item zakaz_add"><b>Заказ</b>Новый заказ для продажи изделий без установки.<br />При необходимости в будущем заказ можно будет перевести в Установку.</div><div class="item zamer_add"><b>Замер</b>Новая заявка на замер. Указывается дата и время замера. Заявка автоматически попадает в напоминания. Успешный замер переносится на заключение договора.</div><div class="item set_add"><b>Установка</b>Новая заявка на установку изделий.</div>',
butSubmit:""})}).on("click",".zayav_unit",function(){document.location.href=URL+"&p=zayav&d=info&id="+$(this).attr("val")}).on("click","#zayav .ajaxNext",function(){var a=$(this);if(!a.hasClass("busy")){var b=zayavFilter();b.op="zayav_next";b.page=a.attr("val");a.addClass("busy");$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json")}}).on("click","#zayav .filter_clear",function(){$(".find-hide").removeClass("dn");window.zFind.clear();$("#product_id")._select(0);
...
"")+"</div>",butSubmit:"",butCancel:"Закрыть"});$("#day")._calendar({lost:1});$(".st").click(function(){var b=$(this),c={op:"set_status",zayav_id:ZAYAV.id,status:b.attr("val"),day:$("#day").val()||""},d=b.parent();d.hasClass("busy")||(d.addClass("busy"),$.post(AJAX_MAIN,c,function(b){b.success?(a.close(),_msg("Данные изменены!"),document.location.reload()):d.removeClass("busy")},"json"))})}).on("click",".remind_calendar .on",function(){var a=$(this),b=a.parent().parent(),a={op:"remind_day",day:a.attr("val")};
b.hasClass("busy")||(b.addClass("busy"),$.post(AJAX_MAIN,a,function(a){a.success&&($("#remind .left").html(a.html),$("#remind").removeClass("y"));b.removeClass("busy")},"json"))}).on("click","#remind .fmon",function(){var a=$(this);$(".right .remind_calendar").html(a.next().html());$(".goyear span").html(a.html());$("#remind").removeClass("y")}).on("click","#history_next",function(){if(!$(this).hasClass("busy")){var a=$(this),b={op:"history_next",page:$(this).attr("val")};a.addClass("busy");$.post(AJAX_MAIN,
b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json")}}).on("click",".invoice_set",function(){function a(){var a={op:"invoice_set",invoice_id:b.attr("val"),sum:$("#sum").val()};REGEXP_NUMERIC.test(a.sum)?(c.process(),$.post(AJAX_MAIN,a,function(e){e.success?(b.parent().html("<b>"+a.sum+"</b> руб."),c.close(),_msg("Баланс установлен.")):c.abort()},"json")):(c.bottom.vkHint({msg:'<SPAN class="red">Некорректно указана сумма</SPAN>',remove:1,indent:40,show:1,top:-48,left:62}),
$("#sum").focus())}var b=$(this),c=_dialog({width:300,top:60,head:"Установка начального баланса счёта",content:'<table style="border-spacing:10px"><tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="7"> руб.</table>',butSubmit:"Установить",submit:a});$("#sum").focus().keyEnter(a)}).on("click",".income_mon",function(){}).on("click","#money_next",function(){if(!$(this).hasClass("busy")){var a=$(this),b={op:"money_next",page:$(this).attr("val"),limit:$("#money_limit").val(),
client_id:$("#money_client_id").val(),zayav_id:$("#money_zayav_id").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json")}}).on("click","#report_month .yr",function(){$(this).next().toggle()}).on("click","#setup_my .pinset",function(){function a(){var a={op:"setup_my_pinset",pin:$.trim($("#pin").val())};a.pin?3>a.pin.length?(b("Длина пин-кода от 3 до 10 символов"),$("#pin").focus()):(c.process(),$.post(AJAX_MAIN,a,function(a){a.success?
(c.close(),_msg("Пин-код установлен."),document.location.reload()):c.abort()},"json")):(b("Введите пин-код"),$("#pin").focus())}function b(a){c.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:-47,left:52,indent:50,show:1,remove:1})}$(this);var c=_dialog({width:300,head:"Установка нового пин-кода",content:'<table class="setup-tab"><tr><td class="label">Новый пин-код:<td><input id="pin" type="password" maxlength="10" /></table>',butSubmit:"Установить",submit:a});$("#pin").focus().keyEnter(a)}).on("click",
"#setup_my .pinchange",function(){function a(){var a={op:"setup_my_pinchange",oldpin:$.trim($("#oldpin").val()),pin:$.trim($("#pin").val())};a.oldpin&&a.pin?3>a.oldpin.length||3>a.pin.length?b("Длина пин-кода от 3 до 10 символов"):(c.process(),$.post(AJAX_MAIN,a,function(a){a.success?(c.close(),_msg("Пин-код изменён."),document.location.reload()):(c.abort(),b(a.text))},"json")):b("Заполните оба поля")}function b(a){c.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:-47,left:52,indent:50,show:1,
remove:1})}$(this);var c=_dialog({width:300,head:"Изменение пин-кода",content:'<table class="setup-tab"><tr><td class="label">Текущий пин-код:<td><input id="oldpin" type="password" maxlength="10" /><tr><td class="label">Новый пин-код:<td><input id="pin" type="password" maxlength="10" /></table>',butSubmit:"Изменить",submit:a});$("#oldpin").focus().keyEnter(a);$("#pin").keyEnter(a)}).on("click","#setup_my .pindel",function(){function a(){var a={op:"setup_my_pindel",oldpin:$.trim($("#oldpin").val())};
...
butSubmit:"Удалить",submit:function(){for(;"DD"!=a[0].tagName;)a=a.parent();var c={op:"setup_zayavrashod_del",id:a.attr("val")};b.process();$.post(AJAX_MAIN,c,function(a){a.success?($(".spisok").html(a.html),b.close(),_msg("Удалено!"),sortable()):b.abort()},"json")}})}).ready(function(){0<$("#pin-enter").length&&($("#pin").focus().keydown(function(){$(".red").html("&nbsp;")}).keyEnter(pinEnter),$(".vkButton").click(pinEnter));0<$("#client").length&&(window.cFind=$("#find")._search({width:602,focus:1,
enter:1,txt:"Начните вводить данные клиента",func:clientSpisokLoad}),$("#buttonCreate").vkHint({msg:'<B>Внесение нового клиента в базу.</B><br /><br />После внесения Вы попадаете на страницу с информацией о клиенте для дальнейших действий.<br /><br />Клиентов также можно добавлять при <A href="'+URL+'&p=zayav&d=add&back=client">создании новой заявки</A>.',ugol:"right",width:215,top:-38,left:-250,indent:40,delayShow:1E3,correct:0}).click(clientAdd),$("#dolg")._check(clientSpisokLoad),$("#dolg_check").vkHint({msg:"<b>Список должников.</b><br /><br />Выводятся клиенты, у которых баланс менее 0. Также в результате отображается общая сумма долга.",
ugol:"right",width:150,top:-6,left:-185,indent:20,delayShow:1E3,correct:0}));0<$("#clientInfo").length&&($("#dopLinks .link").click(function(){$("#dopLinks .link").removeClass("sel");$(this).addClass("sel");var a=$(this).attr("val");$(".res").css("display","zayav"==a?"block":"none");$("#zayav_filter").css("display","zayav"==a?"block":"none");$("#zayav_spisok").css("display","zayav"==a?"block":"none");$("#income_spisok").css("display","money"==a?"block":"none");$("#remind_spisok").css("display","remind"==
