...
b);return!1}function c(){var a=REGEXP_CENA.test($("#sum").val())?$("#sum").val():0,b=REGEXP_CENA.test($("#avans").val())?$("#avans").val():0,a=a-b;s=_dialog({head:"Разбивка платежа на части",content:'<table class="cut-money"><tr><td class="label">Исходная сумма:<td><u>'+a+'</u> руб.<tr id="cut-add"><td><td><a>Добавить поле</a><tr><td class="label">Итоговая сумма:<td><b id="cut-itog">'+a+"</b> руб.</table>",butSubmit:"Применить",submit:f});m=1;q=[];if(DOG.cut)for(a=DOG.cut.split(","),h=0;h<a.length;h++)b=
a[h].split(":"),q.push([b[0],b[1]]),d();else d();g();$("#cut-add a").click(d)}function d(){var a=q[m-1],a='<tr><td class="label">'+m+'-й платёж:<td><input type="text" class="cutsum" id="i'+m+'" maxlength="7" value="'+(a?a[0]:"")+'"> руб. <input type="hidden" id="d'+m+'" value="'+(a?a[1]:"")+'">';$("#cut-add").before(a);1!=m||q[0]||$("#i1").val($("#cut-itog").html());$("#i"+m).focus().keyup(g);$("#d"+m)._calendar();m++}function g(){var a=$(".cutsum"),b=0,d,c=[];for(h=0;h<a.length;h++)if((d=$.trim(a.eq(h).val()))&&
0!=d){if(!REGEXP_CENA.test(d)){b=!1;break}b+=1*d;c.push(1*d+":"+a.eq(h).next().find("input").val())}!1===b&&(b=Math.round(100*b)/100);$("#cut-itog").html(b);return!1===b?"error":c}function f(){var a=g();"error"==a?s.bottom.vkHint({msg:'<span class="red">Некорректно заполнено поле платежа</span>',top:-47,left:84,indent:50,show:1,remove:1}):(DOG.cut=a.join(),a=a.length,$("#cut").html(a?"Указан"+_end(a,["а","ы"])+" дат"+_end(a,["а","ы"])+" для "+a+_end(a,["-го","-х","-и"])+" платеж"+_end(a,["а","ей"])+
'. <u>Изменить</u><div class="img_del'+_tooltip("Отменить разбивку",-61)+"</div>":p),a&&$("#cut .img_del").click(function(a){a.stopPropagation();DOG.cut="";$("#cut").html(p)}),s.close())}function e(a,b,d){r.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:d?-86:-47,left:d?173:142,indent:50,show:1,remove:1});b&&$("#"+b).focus()}var h,k="Заключение",l="Заключить договор",p="Указать даты очередных платежей",m,s,q;switch(a){default:a="create";case "create":break;case "edit":k="Изменение данных";
...
a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".invoice_set",function(){function a(){var a=[];if(f)for(n=0;n<g;n++){var d=h.eq(n).val();if(REGEXP_CENA.test(d))a.push(CASH[n].id+"="+1*d.replace(",","."));else{b("Некорректно указана сумма");h.eq(n).focus();return}}a={op:"invoice_set",invoice_id:c,sum:f?0:$("#sum").val(),cash:a.join(":")};f||REGEXP_CENA.test(a.sum)?(e.process(),$.post(AJAX_MAIN,a,function(a){a.success?
($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),e.close(),_msg("Баланс установлен.")):e.abort()},"json")):(b("Некорректно указана сумма"),$("#sum").focus())}function b(a){e.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:72})}window.CASH||(window.CASH=[]);var c=$(this).attr("val"),d='<table class="_dialog-tab">',g=CASH.length,f="1"==c&&!!g;if(f){for(n=0;n<g;n++)d+='<tr><td class="label">'+CASH[n].name+':<td><INPUT type="text" class="money" maxlength="11" value="'+
CASH[n].sum+'"> руб.';d+='<tr><td class="label">Счёт <b>'+INVOICE_NAME+'</b>:<td><b id="summa">0</b> руб.'}else d+='<tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="11"> руб.';var e=_dialog({width:320,head:"Установка текущей суммы счёта",content:d+"</table>",butSubmit:"Установить",submit:a}),h=e.content.find("input");h.eq(0).focus();h.keyEnter(a);f&&h.keyup(function(){var a=0;for(n=0;n<g;n++){var b=h.eq(n).val();if(REGEXP_CENA.test(b))a+=1*b.replace(",",".");else{a=
...
(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click","#money_all_check",function(){for(var a=$(this),b=a.find("input").val();!a.hasClass("_money");)a=a.parent();for(var c=a.find("tr"),a=1;a<c.length;a++)c.eq(a).find("input:first")._check(b)}).on("click",".inc ._check",incomeChoiceSum).on("click",".income-show",function(){var a=_dialog({top:20,width:480,head:"Просмотр платежей",load:1,butSubmit:"",butCancel:"Закрыть"}),b=
...
$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};c.category||c.about?c.invoice?REGEXP_CENA.test(c.sum)&&0!=c.sum?(e.process(),$.post(AJAX_MAIN,c,function(a){a.success?(e.close(),_msg("Новый расход внесён."),expenseSpisok()):e.abort()},"json")):(b("Некорректно указана сумма."),$("#sum").focus()):b("Укажите с какого счёта производится оплата."):(b("Выберите категорию или укажите описание."),$("#about").focus())}function b(a){e.bottom.vkHint({msg:'<SPAN class="red">'+
