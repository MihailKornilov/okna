...
c.push(1*d+":"+a.eq(h).next().find("input").val())}!1===b&&(b=Math.round(100*b)/100);$("#cut-itog").html(b);return!1===b?"error":c}function g(){var a=d();"error"==a?s.bottom.vkHint({msg:'<span class="red">Некорректно заполнено поле платежа</span>',top:-47,left:84,indent:50,show:1,remove:1}):(DOG.cut=a.join(),f(),s.close())}function f(){var a=DOG.cut?DOG.cut.split(",").length:0;$("#cut").html(a?"Указан"+_end(a,["а","ы"])+" дат"+_end(a,["а","ы"])+" для "+a+_end(a,["-го","-х","-и"])+" платеж"+_end(a,
...
content:'<table class="cut-money"><tr><td class="label">Исходная сумма:<td><u>'+a+'</u> руб.<tr id="cut-add"><td><td><a>Добавить поле</a><tr><td class="label">Итоговая сумма:<td><b id="cut-itog">'+a+"</b> руб.</table>",butSubmit:"Применить",submit:g});m=1;q=[];if(DOG.cut)for(a=DOG.cut.split(","),h=0;h<a.length;h++)b=a[h].split(":"),q.push([b[0],b[1]]),c();else c();d();$("#cut-add a").click(c)}).vkHint({width:180,msg:"Разбить оставшуюся сумму платежа по заявке на части и создать по ним напоминания.",
delayShow:400,top:-82,left:118});f();$("#preview").click(function(){var a=b("preview");if(a){a.op="dogovor_preview";var d="",c;for(c in a)d+='<input type="hidden" name="'+c+'" value="'+a[c]+'">';$("#preview-form").html(d).submit()}});$("#reason").autosize()},historyFilter=function(){return{op:"history_spisok",limit:$("#history_limit").val(),worker_id:$("#history_worker_id").val(),cat_id:$("#history_cat_id").val(),client_id:$("#history_client_id").val(),zayav_id:$("#history_zayav_id").val()}},historySpisok=
...
incomeChoiceSum=function(){var a,b=$("._money .choice"),c=0,d=0,g=!0,f=[];for(a=0;a<b.length;a++){var e=b.eq(a);1==e.find("input").val()?(c++,d+=1*e.parent().find(".sum").html().replace(" ",""),f.push(e.parent().attr("val"))):g=!1}$(".income_choice_sum").html(c?"Выбран"+_end(c,["","о"])+" <b>"+c+"</b> платеж"+_end(c,["","а","ей"])+" на сумму <b>"+d+"</b> руб.":"");$("#money_all")._check(g);return{count:c,ids:f.join(),sum:d}},expenseFilter=function(){for(var a=[],b=$("#monthList input"),c=1;12>=c;c++)1==
b.eq(c-1).val()&&a.push(c);return{op:"expense_spisok",category:$("#category").val(),worker:$("#worker").val(),invoice_id:$("#invoice_id").val(),year:$("#year").val(),month:a.join()}},expenseSpisok=function(){$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,expenseFilter(),function(a){$("#mainLinks").removeClass("busy");a.success&&($("#spisok").html(a.html),$("#monthList").html(a.mon))},"json")},salaryCheck=function(){var a=$("._check"),b,c,d="",g=0,f=0,e=[];for(b=0;b<a.length;b++)c=a.eq(b),0!=c.find("input").val()&&
"salary_all_check"!=c.attr("id")&&(g++,c=c.parent().parent(),e.push(c.attr("val")),f+=1*c.find(".sum").html());g&&(d="Выбран"+_end(g,["а","о"])+" <b>"+g+"</b> запис"+_end(g,["ь","и","ей"])+" на сумму <b>"+f+'</b> руб.<a class="salary-list" val="'+e.join()+'">Распечатать лист выдачи з/п</a>');$("#salary-sel").html(d)},salarySpisok=function(){if(!$(".headName").hasClass("_busy")){MON=1*$("#salmon").val();YEAR=$("#year").val();var a={op:"salary_spisok",worker_id:WORKER_ID,year:YEAR,mon:MON};$(".headName").addClass("_busy");
...
$.fn.productList=function(a){function b(a){var b=g+f,d=b+"id",e=b+"subid",h=b+"count";p.before('<table id="ptab'+f+'" class="ptab" val="'+f+'"><tr><td class="td"><input type="hidden" id="'+d+'" value="'+(a[0]||0)+'" /><input type="hidden" id="'+e+'" value="'+(a[1]||0)+'" /><td class="td"><input type="text" id="'+h+'" value="'+(a[2]||"")+'" class="count" maxlength="3" /> шт.'+(1<f?'<div class="img_del"></div>':"")+"</table>");var k=$("#ptab"+f);k.find(".img_del").click(function(){k.remove()});$("#"+
d)._select({width:119,title0:"Не указано",spisok:PRODUCT_SPISOK,func:function(a){$("#"+e)._select("remove").val(0);0<a&&PRODUCT_SUB_SPISOK[a]&&c(a,e,h);$("#"+h).val(0<a?1:"").focus()}});c(a[0]||0,e,h);f++}function c(a,b,d){0!=a&&PRODUCT_SUB_SPISOK[a]&&$("#"+b)._select({width:120,title0:"Подвид не указан",spisok:PRODUCT_SUB_SPISOK[a],func:function(){$("#"+d).focus()}})}var d=$(this),g=d.attr("id"),f=1,e;if("string"==typeof a&&"get"==a){a=d.find(".ptab");d=[];for(e=0;e<a.length;e++){var h=a.eq(e),k=
g+h.attr("val"),h=$("#"+k+"id").val(),l=$("#"+k+"subid").val(),k=$("#"+k+"count").val();if(0!=h){if(!REGEXP_NUMERIC.test(k)||0==k)return"count_error";d.push(h+":"+l+":"+k)}}return 0==d.length?!1:d.join()}d.html('<div class="_product-list"><a class="add">Добавить поле</a></div>');var p=d.find(".add");p.click(b);if("object"==typeof a)for(e=0;e<a.length;e++)b(a[e]);else b([]);return d};
$.fn.zayavRashod=function(a){function b(a){var c=d+g,f=c+"cat",e=c+"worker";m.append('<table id="ptab'+g+'" class="ptab" val="'+g+'"><tr><td><input type="hidden" id="'+f+'" value="'+(a[0]||0)+'" /><td class="tddop">'+(a[0]&&ZAYAVEXPENSE_TXT[a[0]]?'<input type="text" class="zrtxt" placeholder="описание не указано" tabindex="'+(10*g-1)+'" value="'+a[1]+'" />':"")+(a[0]&&ZAYAVEXPENSE_WORKER[a[0]]?'<input type="hidden" id="'+e+'" value="'+a[1]+'" />':"")+'<td class="tdsum'+(a[0]?"":" dn")+'"><input type="text" class="zrsum" maxlength="6" tabindex="'+
...
k.find(".zrtxt").focus()):(ZAYAVEXPENSE_WORKER[a]?(k.html('<input type="hidden" id="'+e+'" />'),$("#"+e)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}})):k.html(""),l.focus());l.val("");0<a&&!h.next().hasClass("ptab")&&b([])}});g++}var c=$(this),d=c.attr("id"),g=1,f;if("string"==typeof a&&"get"==a){a=c.find(".ptab");c=[];for(f=0;f<a.length;f++){var e=a.eq(f),h=d+e.attr("val"),k=$("#"+h+"cat").val();$("#"+h+"worker").val();var l=e.find(".zrsum").val(),p="";if(0!=
k){if(!REGEXP_NUMERIC.test(l)||0==l)return"sum_error";ZAYAVEXPENSE_TXT[k]?p=e.find(".zrtxt").val():ZAYAVEXPENSE_WORKER[k]&&(p=$("#"+h+"worker").val());c.push(k+":"+p+":"+l)}}return c.join()}c.html('<div class="_zayav-rashod"></div>');var m=c.find("._zayav-rashod");if("object"==typeof a)for(f=0;f<a.length;f++)b(a[f]);b([]);return c};
...
function(){function a(){var a=[];if(f)for(n=0;n<g;n++){var d=h.eq(n).val();if(REGEXP_CENA.test(d))a.push(CASH[n].id+"="+1*d.replace(",","."));else{b("Некорректно указана сумма");h.eq(n).focus();return}}a={op:"invoice_set",invoice_id:c,sum:f?0:$("#sum").val(),cash:a.join(":")};f||REGEXP_CENA.test(a.sum)?(e.process(),$.post(AJAX_MAIN,a,function(a){a.success?($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),e.close(),_msg("Баланс установлен.")):e.abort()},"json")):(b("Некорректно указана сумма"),
$("#sum").focus())}function b(a){e.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:72})}window.CASH||(window.CASH=[]);var c=$(this).attr("val"),d='<table class="_dialog-tab">',g=CASH.length,f="1"==c&&!!g;if(f){for(n=0;n<g;n++)d+='<tr><td class="label">'+CASH[n].name+':<td><INPUT type="text" class="money" maxlength="11" value="'+CASH[n].sum+'"> руб.';d+='<tr><td class="label">Счёт <b>'+INVOICE_NAME+'</b>:<td><b id="summa">0</b> руб.'}else d+='<tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="11"> руб.';
var e=_dialog({width:320,head:"Установка текущей суммы счёта",content:d+"</table>",butSubmit:"Установить",submit:a}),h=e.content.find("input");h.eq(0).focus();h.keyEnter(a);f&&h.keyup(function(){var a=0;for(n=0;n<g;n++){var b=h.eq(n).val();if(REGEXP_CENA.test(b))a+=1*b.replace(",",".");else{a=0;break}}$("#summa").html(a)})}).on("click","#report.invoice .img_note",function(){var a=_dialog({top:20,width:570,head:"История операций со счётом",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"invoice_history",
...
a.parent();for(var c=a.find("tr"),a=1;a<c.length;a++)c.eq(a).find("input:first")._check(b)}).on("click",".inc ._check",incomeChoiceSum).on("click",".income-show",function(){var a=_dialog({top:20,width:480,head:"Просмотр платежей",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"income_transfer_show",ids:$(this).attr("val")};$.post(AJAX_MAIN,b,function(b){b.success?a.content.html(b.html):a.loadError()},"json")}).on("click",".income-confirm",function(){var a=_dialog({top:20,width:480,head:"Подтверждение платежей",
...
'<div class="income_choice_sum"></div>'):a.loadError()},"json")}).on("click",".income-add",function(){function a(){var a={op:"income_add",from:OPL.from,type:$("#income_opl").val(),confirm:$("#confirm").val(),sum:$("#sum").val(),zayav_id:$("#zayav_id").val()||0,client_id:OPL.client_id||0,prim:$.trim($("#prim").val())};0==a.type?b("Не указан вид платежа"):REGEXP_CENA.test(a.sum)&&0!=a.sum?$("#zayav_id").length&&0==a.zayav_id&&!a.prim?(b("Если не выбрана заявка, необходимо указать комментарий"),$("#prim").focus()):
$("#zayav_id").length||a.prim?(d.process(),$.post(AJAX_MAIN,a,function(a){if(a.success)switch(d.close(),_msg("Платёж успешно внесён!"),OPL.from){case "client":$("#income_spisok").html(a.html);$(".left:first").html(a.balans);break;case "zayav":$("#income_spisok").html(a.html);break;case "income":incomeSpisok()}else d.abort()},"json")):(b("Необходимо указать комментарий"),$("#prim").focus()):(b("Некорректно указана сумма."),$("#sum").focus())}function b(a){d.bottom.vkHint({msg:'<SPAN class="red">'+
a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:135})}var c='<table class="income-add-tab">'+("income"!=OPL.from?'<tr><td class="label">Клиент:<td>'+OPL.client_fio+'<tr><td class="label">Заявка:<td><input type="hidden" id="zayav_id" value="'+(OPL.zayav_id?OPL.zayav_id:0)+'">'+(OPL.zayav_id?"<b>№"+OPL.zayav_id+"</b>":""):"")+'<tr><td class="label">Вид платежа:<td><input type="hidden" id="income_opl"><a href="'+URL+'&p=setup&d=income" class="img_edit'+_tooltip("Настройка видов платежей",-85)+'</a><tr class="tr_confirm dn"><td class="label">Подтверждение:<td><input type="hidden" id="confirm"><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" maxlength="11"> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="100"></table>',
...
$(".transfer").click(function(){function a(){var b={op:"invoice_transfer",from:1*$("#from").val(),to:1*$("#to").val(),sum:$("#sum").val(),ids:$("#ids").val()};b.from?b.to?b.from==b.to?c("Выберите другой счёт"):REGEXP_CENA.test(b.sum)&&0!=b.sum?(f.process(),$.post(AJAX_MAIN,b,function(a){a.success?($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),$("#transfer-spisok").html(a.t),f.close(),_msg("Перевод произведён.")):f.abort()},"json")):(c("Некорректно введена сумма"),$("#sum").focus()):c("Выберите счёт-получатель"):
c("Выберите счёт-отправитель")}function c(a,b){f.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:b?-95:-47,left:b?197:92,indent:50,show:1,remove:1})}$(this);var f=_dialog({width:380,head:"Перевод между счетами",content:'<table class="invoice-transfer"><tr><td class="label">Со счёта:<td><input type="hidden" id="from" /><tr><td class="label">На счёт:<td><input type="hidden" id="to" /><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" /> руб. <a class="income-choice">Выбрать платежи</a><input type="hidden" id="ids" /></table>',
