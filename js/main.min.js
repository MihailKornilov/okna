...
a.success&&($("#zayav .result").html(a.result),$("#zayav #spisok").html(a.spisok))},"json")},dogovorCreate=function(a){function b(d){var b={id:DOG.id,zayav_id:ZAYAV.id,fio:$("#fio").val(),adres:$("#adres").val(),pasp_seria:$("#pasp_seria").val(),pasp_nomer:$("#pasp_nomer").val(),pasp_adres:$("#pasp_adres").val(),pasp_ovd:$("#pasp_ovd").val(),pasp_data:$("#pasp_data").val(),nomer:$("#nomer").val(),data_create:$("#data_create").val(),sum:$("#sum").val(),avans:$("#avans").val(),reason:"reneg"==a?$("#reason").val():
""};if(b.fio)if(REGEXP_NUMERIC.test(b.nomer)&&0!=b.nomer)if(REGEXP_CENA.test(b.sum)&&0!=b.sum)if(b.avans&&!REGEXP_CENA.test(b.avans))c("Некорректно указан авансовый платёж","avans",d);else{if("reneg"!=a||b.reason)return b;c("Не указана причина перезаключения договора","reason",d)}else c("Некорректно указана сумма по договору","sum",d);else c("Некорректно указан номер договора","nomer",d);else c("Не указано Фио клиента","fio",d);return!1}function c(a,b,d){g.bottom.vkHint({msg:'<span class="red">'+
a+"</span>",top:d?-86:-47,left:d?173:142,indent:50,show:1,remove:1});b&&$("#"+b).focus()}var d="Заключение",e="Заключить договор";switch(a){default:a="create";case "create":break;case "edit":d="Изменение данных";e="Применить";break;case "reneg":d="Перезаключение",e="Перезаключить договор"}var g=_dialog({width:480,top:10,head:d+" договора",content:'<table class="zayav-dogovor"><tr><td class="label">Фио клиента:<td><input type="text" id="fio" value="'+DOG.fio+'" /><tr><td class="label">Адрес:<td><input type="text" id="adres" value="'+
...
(DOG.avans?DOG.avans:"")+'" /> руб. <span class="prim">(не обязательно)</span>'+("reneg"==a?'<tr><td class="label">Причина перезаключения:<td><input type="text" id="reason" />':"")+'<tr><td colspan="2"><a id="preview">Предварительный просмотр</a><form action="'+AJAX_MAIN+'" method="post" id="preview-form" target="_blank"></form></table>',butSubmit:e,submit:function(){var d=b();d&&(d.op="dogovor_"+a,g.process(),$.post(AJAX_MAIN,d,function(a){a.success?(g.close(),_msg("Договор заключен."),document.location.reload()):
(g.abort(),c(a.text))},"json"))}});$("#data_create")._calendar({lost:1});$("#preview").click(function(){var a=b("preview");if(a){a.op="dogovor_preview";var d="",c;for(c in a)d+='<input type="hidden" name="'+c+'" value="'+a[c]+'">';$("#preview-form").html(d).submit()}});$("#reason").autosize()},historyFilter=function(){return{op:"history_spisok",limit:$("#history_limit").val(),worker_id:$("#history_worker_id").val(),cat_id:$("#history_cat_id").val(),client_id:$("#history_client_id").val(),zayav_id:$("#history_zayav_id").val()}},
historySpisok=function(a,b){var c=historyFilter();c[b]=a;$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,c,function(a){$("#mainLinks").removeClass("busy");a.success&&$(".left").html(a.html)},"json")},incomeSpisok=function(){var a={op:"income_spisok",day:$(".selected").val(),income_id:$("#income_id").val(),worker_id:$("#worker_id").val()};$(".inc-path").addClass("_busy");$.post(AJAX_MAIN,a,function(a){$(".inc-path").removeClass("_busy");a.success&&($(".inc-path").html(a.path),$("#spisok").html(a.html))},
"json")},expenseFilter=function(){for(var a=[],b=$("#monthList input"),c=1;12>=c;c++)1==b.eq(c-1).val()&&a.push(c);return{op:"expense_spisok",category:$("#category").val(),worker:$("#worker").val(),invoice_id:$("#invoice_id").val(),year:$("#year").val(),month:a.join()}},expenseSpisok=function(){$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,expenseFilter(),function(a){$("#mainLinks").removeClass("busy");a.success&&($("#spisok").html(a.html),$("#monthList").html(a.mon))},"json")},setupRulesSet=
function(a,b){$.post(AJAX_MAIN,{op:"setup_rules_set",viewer_id:RULES_VIEWER_ID,value:b,action:a},function(){},"json")};
...
title:"Изменение данных договора"},{uid:2,title:"Перезаключение"},{uid:3,title:"Расторжение"}],nosel:1,func:function(a){1==a&&dogovorCreate("edit");2==a&&dogovorCreate("reneg")}}),$(".zakaz_edit").click(function(){function a(){var c,d={op:"zakaz_edit",zayav_id:ZAYAV.id,product:$("#product").productList("get"),zakaz_txt:$("#zakaz_txt").val(),nomer_vg:$("#nomer_vg").val(),nomer_g:$("#nomer_g").val(),nomer_d:$("#nomer_d").val()};d.product||d.zakaz_txt?"count_error"==d.product?c="Некорректно введено количество изделий":
(b.process(),$.post(AJAX_MAIN,d,function(a){a.success?(b.close(),_msg("Данные изменены!"),document.location.reload()):b.abort()},"json")):c="Необходимо выбрать изделие или вписать заказ вручную";c&&b.bottom.vkHint({msg:'<SPAN class="red">'+c+"</SPAN>",top:-47,left:161,indent:40,show:1,remove:1})}var b=_dialog({width:500,top:30,head:"Заказ №"+ZAYAV.id+" - Редактирование",content:'<table class="zayav-info-edit"><tr><td class="label">Клиент:      <td>'+ZAYAV.client_fio+'<tr><td class="label topi">Изделие:<td id="product"><tr><td><td><input type="text" id="zakaz_txt" placeholder="либо укажите содержание заказа вручную.." maxlength="300" value="'+
...
$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};d.category||d.about?d.invoice?REGEXP_CENA.test(d.sum)&&0!=d.sum?(c.process(),$.post(AJAX_MAIN,d,function(a){a.success?(c.close(),_msg("Новый расход внесён."),expenseSpisok()):c.abort()},"json")):(b("Некорректно указана сумма."),$("#sum").focus()):b("Укажите с какого счёта производится оплата."):(b("Выберите категорию или укажите описание."),$("#about").focus())}function b(a){c.bottom.vkHint({msg:'<SPAN class="red">'+
...
spisok:INVOICE_SPISOK,func:function(){$("#sum").focus()}});$("#sum,#about").keyEnter(a)}),$("#category")._select({width:160,title0:"Любая категория",spisok:EXPENSE_SPISOK,func:expenseSpisok}),$("#worker")._select({width:160,title0:"Все сотрудники",spisok:WORKERS,func:expenseSpisok}),$("#invoice_id")._select({width:160,title0:"Любой счёт",spisok:INVOICE_SPISOK,func:expenseSpisok}),$("#year").years({func:expenseSpisok,center:function(){var a=$("#monthList input"),c=0;for(b=1;12>=b;b++)if(0==a.eq(b-
1).val()){c=1;break}for(b=1;12>=b;b++)$("#c"+b)._check(c);expenseSpisok()}}));$("#report.invoice").length&&$(".transfer").click(function(){function a(){var b={op:"invoice_transfer",from:1*$("#from").val(),to:1*$("#to").val(),sum:$("#sum").val(),cash:window.CASH_SPISOK?1:0,prim:$("#prim").val()};b.from?b.to?b.from==b.to?c("Выберите другой счёт"):REGEXP_CENA.test(b.sum)&&0!=b.sum?(g.process(),$.post(AJAX_MAIN,b,function(a){a.success?($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),$("#transfer-spisok").html(a.t),
g.close(),_msg("Перевод произведён.")):g.abort()},"json")):(c("Некорректно введена сумма"),$("#sum").focus()):c("Выберите счёт-получатель"):c("Выберите счёт-отправитель")}function c(a){g.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:-47,left:82,indent:50,show:1,remove:1})}$(this);var g=_dialog({head:"Перевод между счетами",content:'<table class="invoice-transfer"><tr><td class="label">Со счёта:<td><input type="hidden" id="from" /><tr><td class="label">На счёт:<td><input type="hidden" id="to" /><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" /> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="50" /></table>',
butSubmit:"Применить",submit:a});if(window.CASH_SPISOK&&!window.CSMOVE){for(b=0;b<INVOICE_SPISOK.length;b++)1!=INVOICE_SPISOK[b].uid&&CASH_SPISOK.push(INVOICE_SPISOK[b]);INVOICE_SPISOK=CASH_SPISOK;window.CSMOVE=1}$("#from")._select({width:230,title0:"Не выбран",spisok:INVOICE_SPISOK,func:function(){$("#sum").focus()}});$("#to")._select({width:230,title0:"Не выбран",spisok:INVOICE_SPISOK,func:function(){$("#sum").focus()}});$("#sum").focus();$("#sum,#prim").keyEnter(a)});$("#setup_rules").length&&
($(".gtab-save").click(function(){function a(b){c.vkHint({msg:'<SPAN class="red">'+b+"</SPAN>",top:-57,left:-6,indent:40,show:1,remove:1})}var b={op:"setup_worker_save",viewer_id:RULES_VIEWER_ID,first_name:$("#first_name").val(),last_name:$("#last_name").val(),post:$("#post").val()},c=$(this);b.first_name?b.last_name?(c.addClass("busy"),$.post(AJAX_MAIN,b,function(a){c.removeClass("busy");a.success&&_msg("Сохранено.")},"json")):(a("Не указана фамилия"),$("#last_name").focus()):(a("Не указано имя"),
$("#first_name").focus())}),$(".pin-clear").click(function(){var a={op:"setup_worker_pinclear",viewer_id:RULES_VIEWER_ID},b=$(this);b.addClass("busy");$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");a.success&&_msg("Пин-код сброшен.")},"json")}),$("#rules_appenter")._check(function(a,b){$(".app-div")[(0==a?"add":"remove")+"Class"]("dn");setupRulesSet(a,b);$("#rules_worker")._check(0);$("#rules_product")._check(0);$("#rules_income")._check(0);$("#rules_zayavrashod")._check(0);$("#rules_historyshow")._check(0);
$("#rules_money")._check(0);$("#rules_cash")._check(0)}),$("#rules_worker")._check(setupRulesSet),$("#rules_rekvisit")._check(setupRulesSet),$("#rules_product")._check(setupRulesSet),$("#rules_income")._check(setupRulesSet),$("#rules_zayavrashod")._check(setupRulesSet),$("#rules_historyshow")._check(setupRulesSet),$("#rules_money")._check(setupRulesSet),$("#rules_cash")._check(setupRulesSet));$("#setup_rekvisit").length&&$(".vkButton").click(function(){var a=$(this),b={op:"setup_rekvisit",org_name:$("#org_name").val(),
ogrn:$("#ogrn").val(),inn:$("#inn").val(),kpp:$("#kpp").val(),yur_adres:$("#yur_adres").val(),telefon:$("#telefon").val(),ofice_adres:$("#ofice_adres").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){a.removeClass("busy");b.success&&_msg("Информация сохранена.")},"json")})});
