...
$.fn.zayavRashod=function(a){function b(a){var c=d+f,e=c+"cat",g=c+"worker";p.append('<table id="ptab'+f+'" class="ptab" val="'+f+'"><tr><td><input type="hidden" id="'+e+'" value="'+(a[0]||0)+'" /><td class="tddop">'+(a[0]&&ZAYAVEXPENSE_TXT[a[0]]?'<input type="text" class="zrtxt" placeholder="описание не указано" tabindex="'+(10*f-1)+'" value="'+a[1]+'" />':"")+(a[0]&&ZAYAVEXPENSE_WORKER[a[0]]?'<input type="hidden" id="'+g+'" value="'+a[1]+'" />':"")+'<td class="tdsum'+(a[0]?"":" dn")+'"><input type="text" class="zrsum" maxlength="6" tabindex="'+
10*f+'" value="'+(a[2]||"")+'" />руб.</table>');var h=$("#ptab"+f),k=h.find(".tddop"),l=h.find(".zrsum");a[0]&&ZAYAVEXPENSE_WORKER[a[0]]&&$("#"+g)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}});$("#"+e)._select({width:120,title0:"Категория",spisok:ZAYAVEXPENSE_SPISOK,func:function(a){h.find(".tdsum")[(0<a?"remove":"add")+"Class"]("dn");ZAYAVEXPENSE_TXT[a]?(k.html('<input type="text" class="zrtxt" placeholder="описание не указано" tabindex="'+(10*f-11)+'" />'),
k.find(".zrtxt").focus()):(ZAYAVEXPENSE_WORKER[a]?(k.html('<input type="hidden" id="'+g+'" />'),$("#"+g)._select({width:150,title0:"Сотрудник",spisok:WORKER_SPISOK,func:function(){l.focus()}})):k.html(""),l.focus());l.val("");0<a&&!h.next().hasClass("ptab")&&b([])}});f++}var c=$(this),d=c.attr("id"),f=1,e;if("string"==typeof a&&"get"==a){a=c.find(".ptab");c=[];for(e=0;e<a.length;e++){var g=a.eq(e),h=d+g.attr("val"),k=$("#"+h+"cat").val();$("#"+h+"worker").val();var l=g.find(".zrsum").val(),m="";if(0!=
k){if(!REGEXP_NUMERIC.test(l)||0==l)return"sum_error";ZAYAVEXPENSE_TXT[k]?m=g.find(".zrtxt").val():ZAYAVEXPENSE_WORKER[k]&&(m=$("#"+h+"worker").val());c.push(k+":"+m+":"+l)}}return c.join()}c.html('<div class="_zayav-rashod"></div>');var p=c.find("._zayav-rashod");if("object"==typeof a)for(e=0;e<a.length;e++)b(a[e]);b([]);return c};
...
CASH[n].sum+'"> руб.';d+='<tr><td class="label">Счёт <b>'+INVOICE_NAME+'</b>:<td><b id="summa">0</b> руб.'}else d+='<tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="11"> руб.';var g=_dialog({width:320,head:"Установка текущей суммы счёта",content:d+"</table>",butSubmit:"Установить",submit:a}),h=g.content.find("input");h.eq(0).focus();h.keyEnter(a);e&&h.keyup(function(){var a=0;for(n=0;n<f;n++){var b=h.eq(n).val();if(REGEXP_CENA.test(b))a+=1*b.replace(",",".");else{a=
