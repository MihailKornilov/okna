...
$("#preview-form").html(c).submit()}});$("#reason").autosize()},remindSpisok=function(a){var b=$("#remind").hasClass("y"),c=$("#remind .right ._calendarFilter");a={op:"remind_spisok",day:c.find(".selected").val(),status:$("#status").val(),"private":$("#private").val()};b&&$("#remind").removeClass("y");$.post(AJAX_MAIN,a,function(a){a.success&&($(".remind_spisok").html(a.html),b&&c.html(a.cal))},"json")},incomeSpisok=function(){var a={op:"income_spisok",day:$(".selected").val(),income_id:$("#income_id").val(),
worker_id:window.WORKERS?$("#worker_id").val():0,deleted:$("#deleted").val()};$(".inc-path").addClass("_busy");$.post(AJAX_MAIN,a,function(a){$(".inc-path").removeClass("_busy");a.success&&($(".inc-path").html(a.path),$("#spisok").html(a.html))},"json")},incomeChoiceSum=function(){var a,b=$("._money .choice"),c=0,d=0,e=!0,f=[];for(a=0;a<b.length;a++){var h=b.eq(a);1==h.find("input").val()?(c++,d+=1*h.parent().find(".sum").html().replace(" ",""),f.push(h.parent().attr("val"))):e=!1}$(".income_choice_sum").html(c?
"Выбран"+_end(c,["","о"])+" <b>"+c+"</b> платеж"+_end(c,["","а","ей"])+" на сумму <b>"+d+"</b> руб.":"");$("#money_all")._check(e);return{count:c,ids:f.join(),sum:d}},expenseFilter=function(){for(var a=[],b=$("#monthList input"),c=1;12>=c;c++)1==b.eq(c-1).val()&&a.push(c);return{op:"expense_spisok",category:$("#category").val(),worker:$("#worker").val(),invoice_id:$("#invoice_id").val(),year:$("#year").val(),month:a.join()}},expenseSpisok=function(){$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,
expenseFilter(),function(a){$("#mainLinks").removeClass("busy");a.success&&($("#spisok").html(a.html),$("#monthList").html(a.mon))},"json")},salaryCheck=function(){var a=$("._check"),b,c,d="&nbsp;",e=0,f=0,h=[];for(b=0;b<a.length;b++)c=a.eq(b),"salary_all_check"!=c.attr("id")&&0!=c.find("input").val()&&(e++,c=c.parent().parent(),h.push(c.attr("val")),f+=1*c.find(".sum").html());e&&(d="Выбран"+_end(e,["а","о"])+" <b>"+e+"</b> запис"+_end(e,["ь","и","ей"])+' на сумму <b id="salary-sum">'+f+'</b> руб.<a class="salary-list-create" val="'+
...
a.success&&b.html(a.html)},"json")))}).on("click",".invoice_set",function(){function a(){var a=[];if(f)for(n=0;n<e;n++){var d=g.eq(n).val();if(REGEXP_CENA.test(d))a.push(CASH[n].id+"="+1*d.replace(",","."));else{b("Некорректно указана сумма");g.eq(n).focus();return}}a={op:"invoice_set",invoice_id:c,sum:f?0:$("#sum").val(),cash:a.join(":")};f||REGEXP_CENA.test(a.sum)?(h.process(),$.post(AJAX_MAIN,a,function(a){a.success?($("#cash-spisok").html(a.c),$("#invoice-spisok").html(a.i),h.close(),_msg("Баланс установлен.")):
h.abort()},"json")):(b("Некорректно указана сумма"),$("#sum").focus())}function b(a){h.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,left:72})}window.CASH||(window.CASH=[]);var c=$(this).attr("val"),d='<table class="_dialog-tab">',e=CASH.length,f="1"==c&&!!e;if(f){for(n=0;n<e;n++)d+='<tr><td class="label">'+CASH[n].name+':<td><INPUT type="text" class="money" maxlength="11" value="'+CASH[n].sum+'"> руб.';d+='<tr><td class="label">Счёт <b>'+INVOICE_NAME+'</b>:<td><b id="summa">0</b> руб.'}else d+=
'<tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="11"> руб.';var h=_dialog({width:320,head:"Установка текущей суммы счёта",content:d+"</table>",butSubmit:"Установить",submit:a}),g=h.content.find("input");g.eq(0).focus();g.keyEnter(a);f&&g.keyup(function(){var a=0;for(n=0;n<e;n++){var b=g.eq(n).val();if(REGEXP_CENA.test(b))a+=1*b.replace(",",".");else{a=0;break}}$("#summa").html(a)})}).on("click","#report.invoice .img_note",function(){var a=_dialog({top:20,width:570,
...
$("#invoice-spisok").html(b.i),a.close(),_msg("Платежи подтверждены.")):a.abort()},"json")):a.bottom.vkHint({msg:'<SPAN class="red">Платежи не выбраны</SPAN>',remove:1,indent:50,show:1,top:-48,left:143})}});$.post(AJAX_MAIN,{op:"income_confirm_get"},function(b){b.success?a.content.html(b.html+'<div class="income_choice_sum"></div>'):a.loadError()},"json")}).on("click",".transfer-confirm",function(){var a=_dialog({top:20,width:520,head:"Подтверждение переводов",load:1,butSubmit:"Подтвердить",submit:function(){for(var b=
a.content.find("._check"),c=[],d=0;d<b.length;d++){var e=b.eq(d).find("input");1==e.val()&&c.push(e.attr("id").split("_")[0])}c.length?(b={op:"transfer_confirm",ids:c.join(),about:$("#about").val()},a.process(),$.post(AJAX_MAIN,b,function(b){b.success?($("#invoice-spisok").html(b.i),a.close(),_msg("Переводы подтверждены.")):a.abort()},"json")):a.bottom.vkHint({msg:'<SPAN class="red">Платежи не выбраны</SPAN>',remove:1,indent:50,show:1,top:-48,left:163})}});$.post(AJAX_MAIN,{op:"transfer_confirm_get"},
function(b){b.success?a.content.html(b.html+'<div class="transfer-about">Описание: <input type="text" id="about" /></div>'):a.loadError()},"json")}).on("click",".income-show",function(){var a=_dialog({top:20,width:480,head:"Просмотр платежей",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"income_transfer_show",ids:$(this).attr("val")};$.post(AJAX_MAIN,b,function(b){b.success?a.content.html(b.html):a.loadError()},"json")}).on("click",".income-add",function(){function a(){var a={op:"income_add",from:OPL.from,
type:$("#income_opl").val(),confirm:$("#confirm").val(),sum:$("#sum").val(),zayav_id:$("#zayav_id").val()||0,client_id:OPL.client_id||0,prim:$.trim($("#prim").val())};0==a.type?b("Не указан вид платежа"):REGEXP_CENA.test(a.sum)&&0!=a.sum?$("#zayav_id").length&&0==a.zayav_id&&!a.prim?(b("Если не выбрана заявка, необходимо указать комментарий"),$("#prim").focus()):$("#zayav_id").length||a.prim?(d.process(),$.post(AJAX_MAIN,a,function(a){if(a.success)switch(d.close(),_msg("Платёж успешно внесён!"),OPL.from){case "client":$("#income_spisok").html(a.html);
...
window._calendarFilter=remindSpisok,$("#status")._radio(remindSpisok),$("#private")._check(remindSpisok));$("#report.history").length&&($("#viewer_id_add")._select({width:160,title0:"Все сотрудники",spisok:WORKERS,func:_history}),$("#action")._select({width:160,title0:"Любая категория",spisok:HISTORY_GROUP,func:_history}));$("#report.income").length&&(window._calendarFilter=incomeSpisok,$("#income_id")._select({width:160,title0:"Любые платежи",spisok:INCOME_SPISOK,func:incomeSpisok}),window.WORKERS&&
