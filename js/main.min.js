...
var c=_dialog({width:380,head:"Редактирование расхода",load:1,butSubmit:"Сохранить",submit:function(){var b={id:d,op:"expense_edit",category:1*$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};b.category||b.about?b.invoice?REGEXP_NUMERIC.test(b.sum)?(c.process(),$.post(AJAX_MAIN,b,function(a){a.success?(c.close(),_msg("Расход изменён."),expenseSpisok()):c.abort()},"json")):(a("Некорректно указана сумма."),$("#sum").focus()):a("Укажите с какого счёта производится оплата."):
