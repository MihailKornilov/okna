...
butSubmit:"Применить",submit:a});$("#sum").focus().keyEnter(a)}).on("click",".salary ._next",function(){var a=$(this),b={op:"salary_spisok",page:$(this).attr("val"),worker_id:WORKER_ID};a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".salary ._check",function(){var a=$("._check"),b,c,d="",g=0,e=0,f=[];for(b=0;b<a.length;b++)c=a.eq(b),0!=c.find("input").val()&&(g++,c=c.parent().parent(),f.push(c.attr("val")),
e+=1*c.find(".sum").html());g&&(d="Выбран"+_end(g,["","о"])+" <b>"+g+"</b> платеж"+_end(g,["","а","ей"])+" на сумму <b>"+e+'</b> руб.<a class="salary-list" val="'+f.join()+'">Распечатать лист выдачи з/п</a>');$("#salary-sel").html(d)}).on("click",".salary-list",function(){var a=$(this).attr("val");a&&(location.href=SITE+"/view/salary_list.php?"+VALUES+"&worker_id="+WORKER_ID+"&ids="+a)}).ready(function(){$("#pin-enter").length&&($("#pin").focus().keydown(function(){$(".red").html("&nbsp;")}).keyEnter(pinEnter),
...
$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};c.category||c.about?c.invoice?REGEXP_CENA.test(c.sum)&&0!=c.sum?(f.process(),$.post(AJAX_MAIN,c,function(a){a.success?(f.close(),_msg("Новый расход внесён."),expenseSpisok()):f.abort()},"json")):(b("Некорректно указана сумма."),$("#sum").focus()):b("Укажите с какого счёта производится оплата."):(b("Выберите категорию или укажите описание."),$("#about").focus())}function b(a){f.bottom.vkHint({msg:'<SPAN class="red">'+
