...
c+'" value="'+d[c]+'">';$("#preview-form").html(b).submit()}});$("#reason").autosize()},incomeSpisok=function(){var a={op:"income_spisok",day:$(".selected").val(),income_id:$("#income_id").val(),worker_id:$("#worker_id").val()};$(".inc-path").addClass("_busy");$.post(AJAX_MAIN,a,function(a){$(".inc-path").removeClass("_busy");a.success&&($(".inc-path").html(a.path),$("#spisok").html(a.html))},"json")},expenseFilter=function(){for(var a=[],b=$("#monthList input"),c=1;12>=c;c++)1==b.eq(c-1).val()&&
a.push(c);return{op:"expense_spisok",category:$("#category").val(),worker:$("#worker").val(),invoice_id:$("#invoice_id").val(),year:$("#year").val(),month:a.join()}},expenseSpisok=function(){$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,expenseFilter(),function(a){$("#mainLinks").removeClass("busy");a.success&&($("#spisok").html(a.html),$("#monthList").html(a.mon))},"json")},setupRulesSet=function(a,b){$.post(AJAX_MAIN,{op:"setup_rules_set",viewer_id:RULES_VIEWER_ID,value:b,action:a},function(){},
"json")};
...
d)._select({width:119,title0:"Не указано",spisok:PRODUCT_SPISOK,func:function(a){$("#"+f)._select("remove").val(0);0<a&&PRODUCT_SUB_SPISOK[a]&&c(a,f,h);$("#"+h).val(0<a?1:"").focus()}});c(a[0]||0,f,h);g++}function c(a,b,d){0!=a&&PRODUCT_SUB_SPISOK[a]&&$("#"+b)._select({width:120,title0:"Подвид не указан",spisok:PRODUCT_SUB_SPISOK[a],func:function(){$("#"+d).focus()}})}var d=$(this),e=d.attr("id"),g=1,f;if("string"==typeof a&&"get"==a){a=d.find(".ptab");d=[];for(f=0;f<a.length;f++){var h=a.eq(f),k=
...
else{a=0;break}}$("#summa").html(a)})}).on("click",".income-add",function(){function a(){var a={op:"income_add",from:OPL.from,type:$("#income_opl").val(),sum:$("#sum").val(),zayav_id:$("#zayav_id").val()||0,client_id:OPL.client_id||0,prim:$.trim($("#prim").val())};0==a.type?b("Не указан вид платежа"):REGEXP_CENA.test(a.sum)&&0!=a.sum?$("#zayav_id").length&&0==a.zayav_id&&!a.prim?(b("Если не выбрана заявка, необходимо указать комментарий"),$("#prim").focus()):$("#zayav_id").length||a.prim?(c.process(),
$.post(AJAX_MAIN,a,function(a){if(a.success)switch(c.close(),_msg("Платёж успешно внесён!"),OPL.from){case "client":$("#income_spisok").html(a.html);$(".left:first").html(a.balans);break;case "zayav":$("#income_spisok").html(a.html);break;case "income":incomeSpisok()}else c.abort()},"json")):(b("Необходимо указать комментарий"),$("#prim").focus()):(b("Некорректно указана сумма."),$("#sum").focus())}function b(a){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-48,
left:135})}var c=_dialog({top:60,width:440,head:"Внесение платежа",content:'<table class="income-add-tab">'+("income"!=OPL.from?'<tr><td class="label">Клиент:<td>'+OPL.client_fio+'<tr><td class="label">Заявка:<td><input type="hidden" id="zayav_id" value="'+(OPL.zayav_id?OPL.zayav_id:0)+'">'+(OPL.zayav_id?"<b>№"+OPL.zayav_id+"</b>":""):"")+'<tr><td class="label">Вид платежа:<td><input type="hidden" id="income_opl"><a href="'+URL+'&p=setup&d=income" class="img_edit" title="Перейти к настройке видов платежей"></a><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" maxlength="11"> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="100"></table>',
submit:a});$("#sum").focus();$("#sum,#prim").keyEnter(a);OPL.zayav_spisok&&$("#zayav_id")._select({width:210,title0:"Не указана",spisok:OPL.zayav_spisok});$("#income_opl")._select({width:180,title0:"Не указан",spisok:INCOME_SPISOK,func:function(a){$("#sum").focus()}})}).on("click",".income-del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"income_del",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){a.removeClass("deleting");
b.success&&a.addClass("deleted")},"json")}}).on("click",".income-rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"income_rest",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){b.success&&a.removeClass("deleted")},"json")}).on("click","#income_next",function(){var a=$(this),b={op:"income_next",page:$(this).attr("val"),limit:$("#money_limit").val(),client_id:$("#money_client_id").val(),zayav_id:$("#money_zayav_id").val(),day:$(".selected").val()||""};a.hasClass("busy")||
(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".expense #monthList div",expenseSpisok).on("click",".expense ._next",function(){var a=$(this),b=expenseFilter();b.page=a.attr("val");a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".expense .img_del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");
...
viewer_id:RULES_VIEWER_ID},b=$(this);b.addClass("busy");$.post(AJAX_MAIN,a,function(a){b.removeClass("busy");a.success&&_msg("Пин-код сброшен.")},"json")}),$("#rules_appenter")._check(function(a,b){$(".app-div")[(0==a?"add":"remove")+"Class"]("dn");setupRulesSet(a,b);$("#rules_worker")._check(0);$("#rules_product")._check(0);$("#rules_income")._check(0);$("#rules_zayavrashod")._check(0);$("#rules_historyshow")._check(0);$("#rules_money")._check(0);$("#rules_cash")._check(0)}),$("#rules_worker")._check(setupRulesSet),
$("#rules_rekvisit")._check(setupRulesSet),$("#rules_product")._check(setupRulesSet),$("#rules_income")._check(setupRulesSet),$("#rules_zayavrashod")._check(setupRulesSet),$("#rules_historyshow")._check(setupRulesSet),$("#rules_money")._check(setupRulesSet),$("#rules_cash")._check(setupRulesSet));$("#setup_rekvisit").length&&$(".vkButton").click(function(){var a=$(this),b={op:"setup_rekvisit",org_name:$("#org_name").val(),ogrn:$("#ogrn").val(),inn:$("#inn").val(),kpp:$("#kpp").val(),yur_adres:$("#yur_adres").val(),
telefon:$("#telefon").val(),ofice_adres:$("#ofice_adres").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){a.removeClass("busy");b.success&&_msg("Информация сохранена.")},"json")})});
