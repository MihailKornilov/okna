...
a.day+'" /><td><INPUT TYPE="hidden" id="zamer_hour" value="'+a.hour+'" /><td> : <td><INPUT TYPE="hidden" id="zamer_min" value="'+a.min+'" /></table>';$(".zayav-zamer-dtime").html(a);$("#zamer_day")._calendar();$("#zamer_hour")._select({width:40,spisok:ZAMER_HOUR});$("#zamer_min")._select({width:40,spisok:ZAMER_MIN});$("#zamer_duration")._select({width:100,spisok:ZAMER_DURATION})},zayavFindFast=function(){var a={op:"zayav_findfast",find:$.trim($("#find input").val())};a.find?($("#mainLinks").addClass("busy"),
$.post(AJAX_MAIN,a,function(a){$("#mainLinks").removeClass("busy");a.success&&($("#zayav .result").html(a.result),$("#zayav #spisok").html(a.spisok))},"json")):zayavSpisok()},zayavFilter=function(){return{category:$("#zayav").attr("val"),product:$("#product_id").val()}},zayavSpisok=function(){var a=zayavFilter();a.op="zayav_spisok";$("#mainLinks").addClass("busy");$.post(AJAX_MAIN,a,function(a){$("#mainLinks").removeClass("busy");a.success&&($("#zayav .result").html(a.result),$("#zayav #spisok").html(a.spisok))},
"json")},dogovorCreate=function(){function a(a){var c={id:DOG.id,zayav_id:ZAYAV.id,fio:$("#fio").val(),adres:$("#adres").val(),pasp_seria:$("#pasp_seria").val(),pasp_nomer:$("#pasp_nomer").val(),pasp_adres:$("#pasp_adres").val(),pasp_ovd:$("#pasp_ovd").val(),pasp_data:$("#pasp_data").val(),nomer:$("#nomer").val(),data_create:$("#data_create").val(),sum:$("#sum").val(),avans:$("#avans").val(),reason:DOG.id?$("#reason").val():""};if(c.fio)if(c.adres)if(REGEXP_NUMERIC.test(c.nomer)&&0!=c.nomer)if(REGEXP_NUMERIC.test(c.sum)&&
0!=c.sum)if(c.avans&&!REGEXP_NUMERIC.test(c.avans))b("Некорректно указан авансовый платёж","avans",a);else if(DOG.id&&!c.reason)b("Не указана причина перезаключения договора","reason",a);else return c;else b("Некорректно указана сумма по договору","sum",a);else b("Некорректно указан номер договора","nomer",a);else b("Не указан адрес","adres",a);else b("Не указано Фио клиента","fio",a);return!1}function b(a,b,g){c.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:g?-86:-47,left:g?141:110,indent:50,
show:1,remove:1});b&&$("#"+b).focus()}var c=_dialog({width:426,top:10,head:(DOG.id?"Перез":"З")+"аключение договора",content:'<table class="zayav-dogovor"><tr><td colspan="2">'+(DOG.id?'<div class="i per">При <b>перезаключении договора</b> удаляются сумма старого договора и авансовый платёж. Применятся данные нового договора. Также будет обновлён баланс клиента.</div>':"")+'<tr><td class="label r">Фио клиента:<td><input type="text" id="fio" value="'+DOG.fio+'" /><tr><td class="label r">Адрес:<td><input type="text" id="adres" value="'+
...
DOG.pasp_data+'" /><tr><td class="label r">Номер договора:<td><input type="text" id="nomer" maxlength="6" value="'+DOG.nomer+'" /><tr><td class="label r">Дата заключения:<td><input type="hidden" id="data_create" value="'+(DOG.data_create?DOG.data_create:"")+'" /><tr><td class="label r">Сумма по договору:<td><input type="text" id="sum" class="money" maxlength="6" value="'+(DOG.sum?DOG.sum:"")+'" /> руб.<tr><td class="label r">Авансовый платёж:<td><input type="text" id="avans" class="money" maxlength="6" value="'+
...
g+'" value="'+b[g]+'">';$("#preview-form").html(c).submit()}});$("#reason").autosize()},setupRulesSet=function(a,b){$.post(AJAX_MAIN,{op:"setup_rules_set",viewer_id:RULES_VIEWER_ID,value:b,action:a},function(){},"json")};
...
a.text+"</SPAN>",top:-58,left:-5,indent:40,remove:1,show:1})},"json")}var c=$(this),d="undefined"!=typeof ZAYAV?ZAYAV.id:c.attr("val"),e=_dialog({width:400,top:30,head:"Изменение статуса замера",load:1,butSubmit:"",butCancel:"Закрыть"});"undefined"==typeof ZAYAV?$.post(AJAX_MAIN,{op:"zamer_info_get",zayav_id:d},function(b){b.success?a(b):e.loadError()},"json"):a(ZAYAV)}).on("click",".set_add",function(){"undefined"==typeof CLIENT&&(CLIENT={id:0,fio:"",adres:""});var a=CLIENT.adres,b=_dialog({width:550,
top:30,head:"Внесение новой заявки на установку",content:'<table class="zayav-add"><tr><td class="label">Клиент:<td><INPUT type="hidden" id="client_id" value="'+CLIENT.id+'"><b>'+CLIENT.fio+'</b><tr><td class="label topi">Изделие:<td id="product"><tr><td class="label">Адрес установки:<td><INPUT type="text" id="adres" maxlength="100" /><INPUT type="hidden" id="homeadres" /><tr><td class="label top">Заметка:\t<td><textarea id="comm"></textarea></table>',submit:function(){var a,c={op:"set_add",client_id:$("#client_id").val(),
...
$("#sum").focus())}var b=$(this),c=_dialog({width:300,top:60,head:"Установка начального баланса счёта",content:'<table style="border-spacing:10px"><tr><td class="label">Сумма:<td><INPUT type="text" class="money" id="sum" maxlength="7"> руб.</table>',butSubmit:"Установить",submit:a});$("#sum").focus().keyEnter(a)}).on("click",".income_mon",function(){function a(){var a={op:"invoice_set",invoice_id:b.attr("val"),sum:$("#sum").val()};REGEXP_NUMERIC.test(a.sum)?(c.process(),$.post(AJAX_MAIN,a,function(e){e.success?
...
$("#sum").focus().keyEnter(a)}).on("click","#money_next",function(){if(!$(this).hasClass("busy")){var a=$(this),b={op:"money_next",page:$(this).attr("val"),limit:$("#money_limit").val(),client_id:$("#money_client_id").val(),zayav_id:$("#money_zayav_id").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json")}}).on("click","#report_month .yr",function(){$(this).next().toggle()}).on("click","#setup_worker .add",function(){function a(){if(!e.hasClass("busy")){d=
0;$(".res").remove();var a={user_ids:$.trim($("#viewer_id").val()),fields:"photo_50",v:5.2};a.user_ids&&(/vk.com/.test(a.user_ids)&&(a.user_ids=a.user_ids.split("vk.com/")[1]),/\?/.test(a.user_ids)&&(a.user_ids=a.user_ids.split("?")[0]),/#/.test(a.user_ids)&&(a.user_ids=a.user_ids.split("#")[0]),e.addClass("busy"),VK.api("users.get",a,function(a){e.removeClass("busy");a.response&&(a=a.response[0],e.after('<table class="res"><tr><td class="photo"><img src='+a.photo_50+'><td class="name">'+a.first_name+
" "+a.last_name+"</table>"),d=a.id)}))}}function b(a,b){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:b,left:90})}var c=_dialog({top:50,width:350,head:"Добавление нового сотрудника",content:'<div id="setup_worker_add"><h1>Укажите адрес страницы пользователя или его<br />ID ВКонтакте:</h1><h2>Формат адреса может быть следующих видов:<br /><u>http://vk.com/id12345</u>, <u>http://vk.com/durov</u>.<br />Либо используйте ID пользователя: <u>id12345</u>, <u>durov</u>, <u>12345</u>.</h2><input type="text" id="viewer_id" /><div class="vkButton"><button>Найти</button></div><a class="manual">Или заполните данные вручную..</a><table class="manual_tab"><tr><td class="label">Имя:<td><input type="text" id="first_name" /><tr><td class="label">Фамилия:<td><input type="text" id="last_name" /><tr><td class="label">Пол:<td><input type="hidden" id="sex" value="0" /><tr><td class="label">Должность:<td><input type="text" id="post" /></table></div>',
...
location.href=URL+"&p=client"):a.abort()},"json")}})}),$("#status").rightLink(clientZayavSpisokLoad));0<$("#zayav").length&&(window.zFind=$("#find")._search({width:153,focus:1,txt:"Быстрый поиск...",enter:1,func:zayavFindFast}),$("#product_id")._select({width:155,title0:"Любые изделия",spisok:PRODUCT_SPISOK,func:zayavSpisok}));0<$(".zayav-info").length&&($(".zinfo").click(function(){$(this).parent().find(".sel").removeClass("sel");$(this).addClass("sel");$(".zayav-info").removeClass("h")}),$(".hist").click(function(){$(this).parent().find(".sel").removeClass("sel");
...
zayav_id:ZAYAV.id},function(a){a.success&&document.location.reload()},"json")},nosel:1}),$(".reneg").click(dogovorCreate),$(".zakaz_edit").click(function(){function a(){var a,d={op:"zakaz_edit",zayav_id:ZAYAV.id,product:$("#product").productList("get"),zakaz_txt:$("#zakaz_txt").val(),nomer_vg:$("#nomer_vg").val(),nomer_g:$("#nomer_g").val(),nomer_d:$("#nomer_d").val()};d.product||d.zakaz_txt?"count_error"==d.product?a="Некорректно введено количество изделий":(b.process(),$.post(AJAX_MAIN,d,function(a){a.success?
(b.close(),_msg("Данные изменены!"),document.location.reload()):b.abort()},"json")):a="Необходимо выбрать изделие или вписать заказ вручную";a&&b.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",top:-47,left:161,indent:40,show:1,remove:1})}var b=_dialog({width:500,top:30,head:"Заказ №"+ZAYAV.id+" - Редактирование",content:'<table class="zayav-info-edit"><tr><td class="label">Клиент:      <td>'+ZAYAV.client_fio+'<tr><td class="label topi">Изделие:<td id="product"><tr><td><td><input type="text" id="zakaz_txt" placeholder="либо укажите содержание заказа вручную.." maxlength="300" value="'+
...
ZAYAV.rashod=b.array,a.close(),_msg("Сохранено.")):a.abort()},"json"))}});$("#zrs").zayavRashod(ZAYAV.rashod)}),$(".zakaz-to-set").click(function(){function a(){var a={op:"zakaz_to_set",zayav_id:ZAYAV.id,adres:$("#adres").val()};a.adres?(b.process(),$.post(AJAX_MAIN,a,function(a){b.abort();a.success&&(b.close(),_msg("Выполнено."),document.location.reload())},"json")):(b.bottom.vkHint({msg:'<SPAN class="red">Не указан адрес установки</SPAN>',top:-48,left:113,indent:40,remove:1,show:1,correct:0}),$("#adres").focus())}
...
top:-75,left:163,indent:60,delayShow:400})}));0<$("#remind").length&&$(".goyear").click(function(){$("#remind").addClass("y")});0<$("#setup_rules").length&&($("#gtab_save").click(function(){function a(a){c.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",top:-57,left:-6,indent:40,show:1,remove:1})}var b={op:"setup_worker_save",viewer_id:RULES_VIEWER_ID,first_name:$("#first_name").val(),last_name:$("#last_name").val(),post:$("#post").val()},c=$(this).parent();b.first_name?b.last_name?(c.addClass("busy"),
$.post(AJAX_MAIN,b,function(a){c.removeClass("busy");a.success&&_msg("Сохранено.")},"json")):(a("Не указана фамилия"),$("#last_name").focus()):(a("Не указано имя"),$("#first_name").focus())}),$("#rules_appenter")._check(function(a,b){$(".app-div")[(0==a?"add":"remove")+"Class"]("dn");$(".setup-div").addClass("dn");setupRulesSet(a,b);$("#rules_setup")._check(0);$("#rules_worker")._check(0);$("#rules_product")._check(0);$("#rules_income")._check(0);$("#rules_zayavrashod")._check(0);$("#rules_historyshow")._check(0)}),
...
$(".vkButton").click(function(){var a=$(this),b={op:"setup_rekvisit",org_name:$("#org_name").val(),ogrn:$("#ogrn").val(),inn:$("#inn").val(),kpp:$("#kpp").val(),yur_adres:$("#yur_adres").val(),telefon:$("#telefon").val(),ofice_adres:$("#ofice_adres").val()};a.addClass("busy");$.post(AJAX_MAIN,b,function(b){a.removeClass("busy");b.success&&_msg("Информация сохранена.")},"json")})});
