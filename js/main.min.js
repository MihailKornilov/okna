...
butSubmit:"Установить",submit:a});$("#sum").focus().keyEnter(a)}).on("click","#report.invoice .img_note",function(){function a(a,b){var c=$(".invoice-history #dopLinks"),f={op:"invoice_history_ostatok",invoice_id:$("#invoice_history_id").val(),year:"ih-year"==b?a:$("#ih-year").val(),mon:"ih-mon"==b?a:$("#ih-mon").val()};c.hasClass("_busy")||(c.addClass("_busy"),$.post(AJAX_MAIN,f,function(a){c.removeClass("_busy");a.success&&$("#ih-spisok").html(a.html)},"json"))}var b=_dialog({top:20,width:570,head:"История операций со счётом",
load:1,butSubmit:"",butCancel:"Закрыть"}),c={op:"invoice_history",invoice_id:$(this).attr("val")};$.post(AJAX_MAIN,c,function(c){c.success?(b.content.html('<div class="invoice-history">'+c.html+"</div>"),$("#ih-year")._select({width:50,spisok:[{uid:2014,title:2014},{uid:2015,title:2015}],func:a}),$("#ih-mon")._select({width:80,spisok:MON_SPISOK,func:a})):b.loadError()},"json")}).on("click",".invoice-history .full,.invoice-history .ostatok",function(){var a=$(this),b=a.parent();if(!a.hasClass("sel")){b.find(".sel").removeClass("sel");
a.addClass("sel");b.addClass("_busy");var c=a.hasClass("full")?"full":"ostatok",a={op:"invoice_history_"+c,invoice_id:$("#invoice_history_id").val(),year:$("#ih-year").val(),mon:$("#ih-mon").val()};$.post(AJAX_MAIN,a,function(a){b.removeClass("_busy");a.success&&($("#ih-spisok").html(a.html),$("#ih-data")[("full"==c?"add":"remove")+"Class"]("dn"))},"json")}}).on("click",".invoice-history .to-day",function(){var a=$(".invoice-history #dopLinks");if(!a.hasClass("_busy")){a.find("a.sel").removeClass("sel");
a.find("a.full").addClass("sel");a.addClass("_busy");var b=$(this),b={op:"invoice_history_full",invoice_id:$("#invoice_history_id").val(),day:b.attr("val")};$.post(AJAX_MAIN,b,function(b){a.removeClass("_busy");b.success&&($("#ih-spisok").html(b.html),$("#ih-data").addClass("dn"))},"json")}}).on("click",".invoice-history .ih-clear",function(){var a=$(".invoice-history #dopLinks");if(!a.hasClass("_busy")){a.addClass("_busy");$(this);var b={op:"invoice_history_full",invoice_id:$("#invoice_history_id").val()};
$.post(AJAX_MAIN,b,function(b){a.removeClass("_busy");b.success&&($("#ih-spisok").html(b.html),$("#ih-data").addClass("dn"))},"json")}}).on("click",".invoice-history ._next",function(){var a=$(this),b={op:"invoice_history",page:a.attr("val"),invoice_id:$("#invoice_history_id").val()};a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click","#money_all_check",function(){for(var a=$(this),b=a.find("input").val();!a.hasClass("_money");)a=
a.parent();for(var c=a.find("tr"),a=1;a<c.length;a++)c.eq(a).find("input:first")._check(b)}).on("click",".inc ._check",incomeChoiceSum).on("click",".transfer-show",function(){var a=_dialog({top:20,width:480,head:"Просмотр подтверждённых переводов",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"transfer_show",ids:$(this).attr("val")};$.post(AJAX_MAIN,b,function(b){b.success?a.content.html(b.html):a.loadError()},"json")}).on("click",".transfer-spisok .img_del",function(){var a=$(this),b=_dialog({head:"Удаление перевода",
content:"<center>Подтвердите удаление перевода.</center>",butSubmit:"Удалить",submit:function(){var c={op:"transfer_del",id:a.attr("val")};b.process();$.post(AJAX_MAIN,c,function(a){a.success?($("#invoice-spisok").html(a.i),$(".transfer-spisok").html(a.t),b.close(),_msg("Перевод удалён.")):b.abort()},"json")}})}).on("click",".transfer-spisok ._next",function(){var a=$(this),b={op:"transfer_spisok",page:a.attr("val")};a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?
a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".income-confirm",function(){var a=_dialog({top:20,width:480,head:"Подтверждение платежей",load:1,butSubmit:"Подтвердить",submit:function(){var b={op:"income_confirm",ids:incomeChoiceSum().ids};b.ids?(a.process(),$.post(AJAX_MAIN,b,function(b){b.success?($("#confirm-info").html(b.confirm),$("#invoice-spisok").html(b.i),a.close(),_msg("Платежи подтверждены.")):a.abort()},"json")):a.bottom.vkHint({msg:'<SPAN class="red">Платежи не выбраны</SPAN>',
remove:1,indent:50,show:1,top:-48,left:143})}});$.post(AJAX_MAIN,{op:"income_confirm_get"},function(b){b.success?a.content.html(b.html+'<div class="income_choice_sum"></div>'):a.loadError()},"json")}).on("click",".transfer-confirm",function(){function a(){for(var a=b.content.find("._check"),d=[],e=0;e<a.length;e++){var g=a.eq(e).find("input");1==g.val()&&d.push(g.attr("id").split("_")[0])}d.length?(a={op:"transfer_confirm",ids:d.join(),about:$("#about").val()},b.process(),$.post(AJAX_MAIN,a,function(a){a.success?
($("#invoice-spisok").html(a.i),$(".transfer-spisok").html(a.t),b.close(),_msg("Переводы подтверждены.")):b.abort()},"json")):b.bottom.vkHint({msg:'<SPAN class="red">Платежи не выбраны</SPAN>',remove:1,indent:50,show:1,top:-48,left:163})}var b=_dialog({top:20,width:520,head:"Подтверждение переводов",load:1,butSubmit:"Подтвердить",submit:a});$.post(AJAX_MAIN,{op:"transfer_confirm_get"},function(c){c.success?(b.content.html(c.html+'<div class="transfer-about">Описание: <input type="text" id="about" /></div>'),
$("#about").keyEnter(a)):b.loadError()},"json")}).on("click",".income-show",function(){var a=_dialog({top:20,width:480,head:"Просмотр платежей",load:1,butSubmit:"",butCancel:"Закрыть"}),b={op:"income_transfer_show",ids:$(this).attr("val")};$.post(AJAX_MAIN,b,function(b){b.success?a.content.html(b.html):a.loadError()},"json")}).on("click",".income-add",function(){function a(){var a={op:"income_add",from:OPL.from,invoice_id:_num($("#invoice_id").val()),confirm:$("#confirm").val(),sum:$("#sum").val(),
zayav_id:$("#zayav_id").val()||0,client_id:OPL.client_id||0,prim:$.trim($("#prim").val()),remind_active:window.REMIND?_num($("#remind_active").val()):0};a.invoice_id?_cena(a.sum)?$("#zayav_id").length&&0==a.zayav_id&&!a.prim?(c.err("Если не выбрана заявка, необходимо указать комментарий"),$("#prim").focus()):$("#zayav_id").length||a.prim?(c.process(),$.post(AJAX_MAIN,a,function(a){if(a.success)switch(c.close(),_msg("Платёж успешно внесён"),OPL.from){case "client":$("#income_spisok").html(a.html);
$(".left:first").html(a.balans);break;case "zayav":zayavInfoMoneyUpdate();$("#income_spisok").html(a.html);a.remind&&$("#remind-spisok").html(a.remind);break;case "income":incomeSpisok()}else c.abort()},"json")):(c.err("Необходимо указать комментарий"),$("#prim").focus()):(c.err("Некорректно указана сумма."),$("#sum").focus()):c.err("Не указан счёт")}var b='<table class="income-add-tab">'+("income"!=OPL.from?'<tr><td class="label">Клиент:<td>'+OPL.client_fio+'<tr><td class="label">Заявка:<td><input type="hidden" id="zayav_id" value="'+
(OPL.zayav_id?OPL.zayav_id:0)+'">'+(OPL.zayav_id?"<b>"+OPL.zayav_head+"</b>":""):"")+'<tr><td class="label">На счёт:<td><input type="hidden" id="invoice_id" /><a href="'+URL+'&p=setup&d=invoice" class="img_edit'+_tooltip("Настройка счетов",-58)+'</a><tr class="tr_confirm dn"><td class="label">Подтверждение:<td><input type="hidden" id="confirm"><tr><td class="label">Сумма:<td><input type="text" id="sum" class="money" maxlength="11"> руб.<tr><td class="label">Комментарий:<td><input type="text" id="prim" maxlength="100">'+
(window.ZAYAV&&REMIND.active?'<tr><td><td><div class="_info"><b>Есть '+REMIND.active+" активн"+_end(REMIND.active,["ое","ых"])+" напоминани"+_end(REMIND.active,["е","я","й"])+'!</b><br /><br /><input type="hidden" id="remind_active" value="0" /></div>':"")+"</table>",c=_dialog({width:440,head:"Внесение платежа",content:b,submit:a});$("#sum").focus();$("#sum,#prim").keyEnter(a);OPL.zayav_spisok&&$("#zayav_id")._select({width:210,title0:"Не указана",spisok:OPL.zayav_spisok});$("#invoice_id")._select({width:180,
title0:"Не указан",spisok:INVOICE_SPISOK,func:function(a){$("#sum").focus();$(".tr_confirm")[(INVOICE_CONFIRM_INCOME[a]?"remove":"add")+"Class"]("dn");$("#confirm")._check(0)}});$("#confirm")._check();$("#confirm_check").vkHint({width:210,msg:"Установите галочку, если платёж нужно внести, но требуется подтверждение о его поступлении на счёт.",top:-96,left:-100});window.ZAYAV&&REMIND.active&&$("#remind_active")._check({name:"отметить выполненным"+_end(REMIND.active,["","и"])})}).on("click",".income-del",
function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"income_del",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){a.removeClass("deleting");b.success&&(window.ZAYAV&&zayavInfoMoneyUpdate(),a.addClass("deleted"))},"json")}}).on("click",".income-rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"income_rest",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){b.success&&(window.ZAYAV&&zayavInfoMoneyUpdate(),
a.removeClass("deleted"))},"json")}).on("click","#income_next",function(){var a=$(this),b={op:"income_next",page:$(this).attr("val"),limit:$("#money_limit").val(),client_id:$("#money_client_id").val(),zayav_id:$("#money_zayav_id").val(),deleted:$("#money_deleted").val(),invoice_id:$("#money_invoice_id").val(),owner_id:$("#money_owner_id").val(),worker_id:$("#money_worker_id").val(),day:$(".selected").val()||""};a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():
a.removeClass("busy")},"json"))}).on("click",".expense #monthList div",expenseSpisok).on("click",".expense ._next",function(){var a=$(this),b=expenseFilter();b.page=a.attr("val");a.hasClass("busy")||(a.addClass("busy"),$.post(AJAX_MAIN,b,function(b){b.success?a.after(b.html).remove():a.removeClass("busy")},"json"))}).on("click",".expense .img_del",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();if(!a.hasClass("deleting")){a.addClass("deleting");var b={op:"expense_del",id:a.attr("val")};
$.post(AJAX_MAIN,b,function(b){a.removeClass("deleting");b.success&&a.addClass("deleted")},"json")}}).on("click",".expense .img_rest",function(){for(var a=$(this);"TR"!=a[0].tagName;)a=a.parent();var b={op:"expense_rest",id:a.attr("val")};$.post(AJAX_MAIN,b,function(b){b.success&&a.removeClass("deleted")},"json")}).on("click",".expense .img_edit",function(){function a(a){c.bottom.vkHint({msg:'<SPAN class="red">'+a+"</SPAN>",remove:1,indent:40,show:1,top:-47,left:101})}for(var b=$(this);"TR"!=b[0].tagName;)b=
b.parent();var c=_dialog({width:380,head:"Редактирование расхода",load:1,butSubmit:"Сохранить",submit:function(){var b={id:d,op:"expense_edit",category:1*$("#cat").val(),about:$("#about").val(),worker:$("#work").val(),invoice:1*$("#invoice").val(),sum:$("#sum").val()};b.category||b.about?b.invoice?REGEXP_NUMERIC.test(b.sum)?(c.process(),$.post(AJAX_MAIN,b,function(a){a.success?(c.close(),_msg("Расход изменён."),expenseSpisok()):c.abort()},"json")):(a("Некорректно указана сумма."),$("#sum").focus()):
a("Укажите с какого счёта производится оплата."):(a("Выберите категорию или укажите описание."),$("#about").focus())}}),d=b.attr("val");$.post(AJAX_MAIN,{op:"expense_get",id:d},function(a){c.content.html('<table id="expense-add-tab"><tr><td class="label">Категория:<td><input type="hidden" id="cat" value="'+a.category+'"><tr class="tr-work '+(EXPENSE_WORKER[a.category]?"":"dn")+'"><td class="label">Сотрудник:<td><input type="hidden" id="work" value="'+a.worker+'"><tr><td class="label">Описание:<td><input type="text" id="about" maxlength="150" value="'+
