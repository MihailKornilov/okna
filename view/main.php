...
function GvaluesCreate() {//Составление файла G_values.js
	$save = //'function _toSpisok(s){var a=[];for(k in s)a.push({uid:k,title:s[k]});return a}'.
		...
		"\n".'EXPENSE_WORKER_ASS='.query_ptpJson("SELECT `id`,`show_worker` FROM `setup_expense` WHERE `show_worker`=1").','.
...
function _invoice($type_id=false) {//Список изделий для заявок
...
function report() {
	...
	switch($d) {
		default: $d = $def;
		case 'history': $left = RULES_HISTORYSHOW ? history_spisok() : _norules(); break;
		case 'money':
...
function history_types($v) {
	switch($v['type']) {
		...
		case 518: return 'В установках: внесение новой категории расходов организации <u>'.$v['value'].'</u>.';
		case 519: return 'В установках: изменение данных категории расходов организации <u>'.$v['value'].'</u>:<div class="changes">'.$v['value1'].'</div>';
		case 520: return 'В установках: удаление данных категории расходов организации <u>'.$v['value'].'</u>.';
...
function _invoiceBalans($invoice_id, $start, $worker_id=0) {// Получение текущего баланса счёта
	$income = query_value("SELECT IFNULL(SUM(`sum`),0) FROM `money` WHERE `deleted`=0 AND `invoice_id`=".($worker_id ? "1 AND `viewer_id_add`=".$worker_id : $invoice_id));
	$from = query_value("SELECT IFNULL(SUM(`sum`),0) FROM `invoice_transfer` WHERE `invoice_from`=".($worker_id ? "1 AND `worker_from`=".$worker_id : $invoice_id));
	$to = query_value("SELECT IFNULL(SUM(`sum`),0) FROM `invoice_transfer` WHERE `invoice_to`=".($worker_id ? "1 AND `worker_to`=".$worker_id : $invoice_id));
...
function invoice() {
	return
		...
		'</div>'.
		'<div id="cash-spisok">'.cash_spisok().'</div>'.
...
function cash_spisok() {
	...
	while($r = mysql_fetch_assoc($q)) {
		$sum = $r['cash'] == -1 ? '' : _invoiceBalans(1, $r['cash'], $r['viewer_id']);
		$send .= '<tr>'.
	...
	$send .= '</table>'.
		'<script type="text/javascript">'.
			'var CASH=['.implode(',', $cash).'],'.
				'CASH_SPISOK=['.implode(',', $cashSpisok).'],'.
...
function invoice_spisok() {
	...
	while($r = mysql_fetch_assoc($q))
		$send .= '<tr>'.
			...
			'<td class="balans">'.
			($r['start'] != -1 ? '<b>'._sumSpace(_invoiceBalans($r['id'], $r['start'])).'</b> руб.' : (VIEWER_ADMIN ? '' : '<a class="invoice_set" val="'.$r['id'].'">Установить текущую сумму</a>')).
...
function expense_right() {
	$sql = "SELECT DISTINCT `worker_id`
			FROM `money`
			WHERE `sum`<0 AND `worker_id`>0";
	$q = query($sql);
	$workers = array();
	while($r = mysql_fetch_assoc($q))
		$workers[] = '{uid:'.$r['worker_id'].',title:"'._viewer($r['worker_id'], 'name').'"}';
...
function expenseMonthSum($v=array()) {
	$filter = array(
		...
		'month' => !empty($v['month']) && preg_match(REGEXP_NUMERIC, $v['month']) ? $v['month'] : intval(strftime('%m')),
	...
	return _radio('monthSum', $mon, $filter['month'], 1);
}//expenseMonthSum()
function expense() {
	$data = expense_spisok();
	return
...
function expenseFilter($v) {
	...
	$send['month'] = $send['year'].'-'.($send['month'] < 10 ? 0 : '').$send['month'];
	unset($send['year']);
	return $send;
}//expenseFilter()
function expense_spisok($filter=array()) {
	...
	$cond = "`deleted`=0
		AND `sum`<0
		AND `dtime_add` LIKE '".$filter['month']."%'".
		($filter['category'] ? ' AND `expense_id`='.$filter['category'] : '').
		($filter['worker'] ? " AND `worker_id`=".$filter['worker'] : '');
